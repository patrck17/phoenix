 ...
 *									*
 * Designed for CircleMUD 3.0			            May  1998	*
 ************************************************************************/
...
/*
 * Any log() message with 'Ack' in it is _really_ bad.
 */
 ...
 * Unset: Log only error messages.				(recommended)
 */
#define BUF_NUKE	(1 << 0)
 ...
 */
ush_int buffer_opt = BUF_NUKE|BUF_OVERBOOT;

/*
 * Set  : Check every pointer in every buffer allocated on every new allocation.
 * Unset: Recommended, unless you are tracking memory corruption at the time.
...
#define LEASE_LIFE	(300 RL_SEC)	/* 2.5 minutes */
 ...
 * decent one.  Of course, it's not perfect for non-powers of 2, but close.
 *
 * MAX_ALLOC - The largest buffer allowed without griping about it.
 ...
 * BUFFER_HASH - The direct size->array mapping number. This is _not_ an
	arbitrary number.
 * CACHE_SIZE - How many data->buffer mappings to maintain. The cache is
 *	useless with BUFFER_SNPRINTF, but that isn't done yet.
 */
#define MAX_ALLOC	(16)	/* 2**16 = 65536 bytes	*/
#define MIN_ALLOC	(6)	/* 2**6  = 64 bytes	*/
#define BUFFER_HASH	(11)	/* Not arbitrary! */
#define CACHE_SIZE	(3 + BUFFER_MEMORY * 7)	/* Cache is 10 with that. */
 ...
 * variable is requested.  This does waste 8 bytes for every malloc() that we
...
static int get_magnitude(unsigned long count);
#if THREADED
...
#define valid_pointer(ptr)	(((unsigned long)(ptr) & 0x8000000))
...
static void buffer_sanity_check(struct buf_data **check)
{
   ...
   for (magnitude = 0; magnitude <= buffer_array[BUF_MAX]; magnitude++)
	 ...
	 log("BUF: SYSERR: %s/%d head corrupted.",
	     check == get_buffer_head() ? "buffer" : "memory", magnitude);
	 ...
	 if (chk->data[chk->req_size] != MAGIC_NUMBER && (die = TRUE))
	    log("BUF: SYSERR: %p (%s:%d) overflowed requested size (%d).",
		chk, chk->who, chk->line, chk->req_size);
	 if (chk->data[chk->size] != MAGIC_NUMBER && (die = TRUE))
	    log("BUF: SYSERR: %p (%s:%d) overflowed real size (%d).", chk,
		chk->who, chk->line, chk->size);
	 }
      }
   if (die)
      buffer_reboot();
 ...
 */
static int get_magnitude(unsigned long y)
{
   ush_int magnitude = buffer_hash(y);

   if (magnitude > buffer_array[BUF_MAX])
      {
      log("BUF: SYSERR: Hash result %d out of range 0-%d.", magnitude, buffer_array[BUF_MAX]);
      magnitude = buffer_array[BUF_MAX];
      }

   if (buffer_opt & BUF_VERBOSE)
      log("BUF: get_magnitude: %ld bytes -> %d", y, magnitude);
...
void init_buffers(void)
{
   ...
   */
   ...
   log("BUF: Buffer cache: %d elements, %d bytes.", CACHE_SIZE,
       sizeof(struct buf_data *) * CACHE_SIZE);
...
static void decrement_all_buffers(void)
{
   ...
   for (magnitude = 0; magnitude <= buffer_array[BUF_MAX]; magnitude++)
	 ...
	 {
	 if (clear->life < 0)	/* We don't want to go negative. */
	    log("BUF: SYSERR: %p from %s:%d has %d life.", clear, 
		clear->who, clear->line, clear->life);
...
static void release_old_buffers(void)
{
   ...
   for (magnitude = 0; magnitude <= buffer_array[BUF_MAX]; magnitude++)
	 ...
	 if (relbuf->type == BT_MALLOC) 
	    {
	    log("BUF: SYSERR: Ack, memory (%p) in buffer list.", relbuf);
	    continue;
	    }
	 ...
	 log("BUF: %s:%d forgot to release %d bytes in %p.", 
	     relbuf->who ? relbuf->who : relbuf->old, relbuf->line,
	     relbuf->size, relbuf);
	 clear_buffer(relbuf);
	 UNLOCK(relbuf);
	 }
...
/* 	 lock_buffers(); */
	 if (traverse == head[magnitude])
	    ...
	    log("BUF: SYSERR: remove_buffer: Don't know what to do with %p.",
		removeme);
/* 	 unlock_buffers(); */
#if THREADED
	 if (LOCKED(removeme) != LOCK_WILL_REMOVE)
   ...
   log("BUF: SYSERR: remove_buffer: Couldn't find the buffer %p from %s:%d.", 
       removeme, removeme->who, removeme->line);
}
...
static void clear_buffer(struct buf_data *clearme)
{
   ...
   */
   if (clearme->data[clearme->req_size] != MAGIC_NUMBER)
      {
      log("BUF: SYSERR: Overflow in buffer %p (%s) from %s:%d.",
	  clearme, clearme->type == BT_MALLOC ? clearme->var : "?", 
	  clearme->who ? clearme->who : clearme->old, clearme->line);
	 ...
	 */
	 log("BUF: %s:%d used %d/%d bytes in %p.",
	     clearme->who, clearme->line, 
	     (clearme->data && *clearme->data ?
	 ...
	 *clearme->data = '\0';
      clearme->who = NULL;
      clearme->line = 0; 
      if(clearme->req_size>clearme->size)
	 log("BUF:SYSERR: WHAT THE HELL! %d/%d/%s",clearme->req_size,
	     clearme->size,clearme->old);
      clearme->req_size = clearme->size;	/* For exit_buffers() check. */
/*       clearme->data[clearme->req_size]=MAGIC_NUMBER; */
      clearme->life = B_FREELIFE;
#if THREADED
      if (LOCKED(clearme) != LOCK_WILL_CLEAR)
	 log("BUF: SYSERR: clear_buffer: Someone cleared lock bit on %p.", 
	     clearme);
#endif
      }
}
...
struct buf_data *detach_buffer(buffer *data, byte type, const char *func, const int line_n)
{
   ...
   if (data == NULL || func == NULL) 
      {
      log("BUF: SYSERR: detach_buffer: Invalid information passed. %s %s %d %d",
	  data?data:"data missing",
	  func?func:"func missing",
	  line_n,(int)type);
      return FALSE;
      }
   ...
   if (clear == NULL) 
      {
      log("BUF: SYSERR: detach_buffer: No buffer->data %p found for %s:%d:%d.",
	  data, func, line_n,type);
#if THREADED
      } 
   else if (LOCKED(clear)) 
      {
      if (LOCKED(clear) == LOCK_WILL_FREE)
	 log("BUF: detach_buffer: Buffer %p slated to be freed.", clear);
      else if (LOCKED(clear) == LOCK_WILL_CLEAR)
	 log("BUF: detach_buffer: Buffer %p already being detached.", clear);
      else if (LOCKED(clear) == LOCK_WILL_REMOVE)
	 log("BUF: detach_buffer: Buffer %p being removed from list.", clear);
      else
	 log("BUF: detach_buffer: Buffer %p already locked.", clear);
#endif
      }
   else if (clear->who == NULL)
      log("BUF: SYSERR: detach_buffer: Buffer %p already released. Locked: %d (%d|%s)(%d|%s)",
	  clear, LOCKED(clear),clear->oline,clear->old,line_n,func);
   else 
      {
      LOCK(clear, LOCK_WILL_CLEAR);
      if (buffer_opt & (BUF_VERBOSE | BUF_DETAIL))
	 log("BUF: %s:%d released %d bytes in '%s' (%p, %d scans) from %s:%d.",
	     func, line_n, clear->size,
	     clear->type == BT_MALLOC ? clear->var : "?", clear, scanned, 
	     clear->who ? clear->who : clear->old, clear->line);
      clear_buffer(clear);
      UNLOCK(clear);
      return clear;
      }
...
static void free_buffer(struct buf_data *f)
{
   if (f == NULL) 
      ...
      log("BUF: SYSERR: free_buffer: Freeing %d byte persistant buffer %p.", 
	  f->size, f);
   else if (f->type == BT_MALLOC) 
      {
      if (buffer_opt & (BUF_VERBOSE | BUF_DETAIL))
	 log("BUF: free_buffer: Freeing %d bytes in '%s' (%p) from %s:%d.",
	     f->size, f->var, f, f->who, f->line);
      }
   else if(buffer_opt&(BUF_VERBOSE|BUF_DETAIL))
      log("BUF: free_buffer: Freeing %d bytes in expired buffer %p.",
	  f->size, f);
...
static struct buf_data *malloc_buffer(size_t size, int type)
{
   ...
   if (!(new_buf = (struct buf_data *)malloc(sizeof(struct buf_data))))
      {
      log("BUF: SYSERR: malloc_buffer: Failed %d byte 'buf_data' malloc().",
...
struct buf_data *find_free_buffer(size_t size)
{
   ...
   for (search = head[magnitude]; search; search = search->next)
	 ...
	 {
	 log("BUF: SYSERR: Ack, memory (%p) in buffer list.", search);
	 continue;
	 }
      if (LOCKED(search))
	 continue;
...
buffer *acquire_buffer(size_t size, int type, const char *varname, const char *who, ush_int line)
{
   ...
   if(size>BUFFER_MAXSIZE)
      log("BUF: SYSERR: %s:%d requested %d bytes for '%s' (OVER MAX %d).",who,
	  line,size,varname,BUFFER_MAXSIZE);
   ...
   if (size == 0)
	 ...
	 allocate_size = BUFFER_MINSIZE;
      ...
      if (buffer_opt & (BUF_VERBOSE | BUF_DETAIL))
	 log("BUF: acquire_buffer: Making a new %ld byte buffer for %s:%d.",
	     allocate_size, who, line);
   ...
   if (*give->data != '\0') 
      {
      log("BUF: SYSERR:a acquire_buffer: Buffer %p is not empty as it ought to be! (%s|%d) DATA:%s", give,give->old,give->line,give->data);
      *give->data = '\0';
      }
   ...
   if (type != BT_MALLOC)	/* FIX HERE MAYBE */
      give->life = B_GIVELIFE;
   else
      give->var = varname;

   if (buffer_opt & (BUF_VERBOSE | BUF_DETAIL))
      log("BUF: %s:%d requested %d bytes for '%s', received %d in %p.", who,
	  line, size, varname ? varname : "a buffer", give->size, give);
   ...
   */
   ...
   if (*give->data != '\0') 
      {
      log("BUF: SYSERR:b acquire_buffer: Buffer %p is not empty as it ought to be! (%s|%d) DATA:%s", give,give->old,give->line,give->data);
      *give->data = '\0';
      }
...
void show_buffers(struct char_data *ch, int buffer_type, int display_type)
{
   ...
   if (display_type == 1 || display_type == 2)
	 ...
	 for (i = 0, bytes = 0, disp = head[magnitude]; disp;
	      disp = disp->next, i++)
	    bytes += disp->size;
	 log("%5d bytes (%2d): %5ld items, %6ld bytes, %5ld overhead.",
	     (1 << size), magnitude, i, bytes, sizeof(struct buf_data) * i);
	 }
      return;
      }
   ...
   for (size = 0; size <= buffer_array[BUF_MAX]; size++)
	 ...
	 if (buffer_type != -1 && buffer_type != disp->type) /* -1 == all */
	    continue;
	 sprintf(buf, "%2ld#%2ld %5dby, %5dsec, %7s, Alloc: %s/%d.(%s|%d)%s",
		 size, ++i, disp->size, disp->life,
		 buf_type[(int)disp->type],
		 disp->who ? disp->who  : "unused",
		 disp->who ? disp->line : disp->oline,
		 disp->who ? ""         : disp->old,
		 disp->oreq_size, ch ? "\r\n" : "");
	 if (ch)
...
ACMD(do_buffer)
{
  ...
  /* Don't need these now. */
   ...
   if (is_abbrev(arg1, "verbose"))
      ...
      send_to_char(ch,(buffer_opt & BUF_VERBOSE) ? "Verbose On.\r\n" : "Verbose Off.\r\n");
      }
   else if (is_abbrev(arg1, "detailed"))
      {
      buffer_opt ^= BUF_DETAIL;
      send_to_char(ch,(buffer_opt & BUF_DETAIL) ? "Detailed On.\r\n" : "Detailed Off.\r\n");
      }
   else if (is_abbrev(arg1, "paranoid"))
      {
      buffer_opt ^= BUF_NUKE;
      send_to_char(ch,(buffer_opt & BUF_NUKE) ? "Now paranoid.\r\n" : "No longer paranoid.\r\n");
      } 
   else if (is_abbrev(arg1, "check"))
      {
      buffer_opt ^= BUF_CHECK;
      send_to_char(ch,(buffer_opt & BUF_CHECK) ? "Checking on.\r\n" : "Not checking.\r\n");
      }
...
{
   if (buffer_opt & BUF_VERBOSE)
      log("BUF: debug_str_dup: %d bytes from %s:%d.", strlen(txt) + 1,
	  func, line);
   return strcpy(acquire_buffer(strlen(txt) + 1, BT_MALLOC, varname, func,
				line), txt);
}

void *debug_calloc(size_t count, size_t size, const char *varname, const char *func, int line)
{
   if (buffer_opt & BUF_VERBOSE)
      log("BUF: debug_calloc: %d*%d bytes from %s:%d.", count, size, 
	  func, line);
   return acquire_buffer(count * size, BT_MALLOC, varname, func, line);
}
...
void *debug_realloc(void *ptr, size_t size, const char *varname, const char *func, int line)
{
   void *xtra;
   if (buffer_opt & BUF_VERBOSE)
      log("BUF: debug_realloc: '%s' (%p) resized to %d bytes for %s:%d.",
	  varname, ptr, size, func, line);
...
static void buffer_unlock(struct buf_data *buf, const char *func, ush_int line)
{
   if (buf == NULL)
      ...
      log("BUF: SYSERR: buffer_unlock: Buffer %p isnt locked, from %s:%d.", buf, func, line);
   else
 ...
 */
char *b2b_cpy(buffer *str1, const buffer *str2) { strcpy(str1->data, 
							 str2->data); }
