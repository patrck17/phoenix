...
if((check = real_mobile(((zone)*100)+(mob))<0)) {\
 if (!mini_mud) { \
  log("assign_kings_castle(): can't find mob #%d.", ((zone)*100)+(mob)); \
  }           \
 } else       \
...
{ 
   ...
   if ((IN_ROOM(ch) == real_room(iIn_room)) && (cmd == iProhibited_direction)) 
      { 
...
{ 
   if (OBJWEAR_FLAGGED(i, ITEM_WEAR_TAKE) && 
       ...
       !(GET_OBJ_VAL(i, 3) && (GET_OBJ_TYPE(i)==ITEM_CONTAINER))) 
      return TRUE; 
   else 
      return FALSE; 
} 
...
{ 
  ...
  /* Find someone suitable to fry ! */ 
   ...
   switch (number(0, 8)) 
	  ...
	  cast_spell(ch, tch, 0, 0, 0,SPELL_FIREBALL,number(6,10)); 
	  break; 
       default: 
	  if (!number(0, 1)) 
	     cast_spell(ch, ch, 0, 0, 0,SPELL_HEAL,number(6,10)); 
	  break; 
      } 
...
{ 
   ...
   switch (path[mindex]) 
	  ...
	  break; 
       ...
       case 'C': 
       case 'D': 
	  act(monolog[path[mindex] - 'A'], FALSE, ch, 0, 0, TO_ROOM); 
	  break; 
       case 'P': 
	  break; 
       case 'W': 
...
{ 
   ...
   if (!cmd) 
      if ((m_tim = find_npc_by_name(ch, "Tim", 3))) 
	 if (FIGHTING(m_tim) && 2 * GET_HIT(m_tim) < GET_HIT(ch)) 
	    do_npc_rescue(ch, m_tim); 
...
{ 
   ...
   if (!cmd) 
      if ((m_tom = find_npc_by_name(ch, "Tom", 3))) 
	 if (FIGHTING(m_tom) && 2 * GET_HIT(m_tom) < GET_HIT(ch)) 
	    do_npc_rescue(ch, m_tom); 
...
{ 
   ...
   for (i = world[IN_ROOM(ch)].contents; i; i = next) 
      { 
      next = i->next_content; 
      if (is_trash(i)) 
	 { 
