...
#define bug(x, y) { mudlogf(CMP,LVL_IMMORT,TRUE,(x), (y));}
		    ...
		    struct obj_data *obj, void *vo, struct char_data *rndm)
{
   ...
   if (*point == '\0') 
      {
      bug ("Mob: %ld null ifchck", mob_index[mob->nr].vnum); 
      return -1;
      }   
  /* skip leading spaces */
   while (*point == ' ')
  ...
  /* get whatever comes before the left paren.. ignore spaces */
   while (*point != '(') 
      if (*point == '\0') 
	 {
	 bug ("Mob: %ld ifchck syntax error", mob_index[mob->nr].vnum); 
	 return -1;
	 }   
      else
	 if (*point == ' ')
   ...
   */
   ...
   if (!str_cmp(buf, "ispc"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug ("Mob: %ld bad argument to 'ispc'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "isnpc"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug ("Mob: %ld bad argument to 'isnpc'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "isgood"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug("Mob: %ld bad argument to 'isgood'",
                 mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "isfight"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug ("Mob: %ld bad argument to 'isfight'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "isimmort"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug ("Mob: %ld bad argument to 'isimmort'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "ischarmed"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  case 'i': return AFF_FLAGGED(mob, AFF_CHARM);
	  case 'n': if (actor)
	     return AFF_FLAGGED(actor, AFF_CHARM);
	  else return -1;
	  case 't': if (vict)
	     return AFF_FLAGGED(vict, AFF_CHARM);
	  else return -1;
	  case 'r': if (rndm)
	     return AFF_FLAGGED(rndm, AFF_CHARM);
	  else return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'ischarmed'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "isfollow"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug ("Mob: %ld bad argument to 'isfollow'", 
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "isaffected"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  default:
	     bug ("Mob: %ld bad argument to 'isaffected'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "hitprcnt"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'hitprcnt'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "inroom"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'inroom'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "sex"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'sex'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "position"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'position'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "level"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'level'",
		  mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "class"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'class'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "goldamt"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'r': if (rndm)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'goldamt'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "objtype"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'p': if (v_obj)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'objtype'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "objval0"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'p': if (v_obj)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'objval0'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "objval1"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'p': if (v_obj)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'objval1'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "objval2"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'p': if (v_obj)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'objval2'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "objval3"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'p': if (v_obj) 
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'objval3'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "number"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'o': if (obj)
	     {
	     lhsvl = obj_index[obj->item_number].vnum;
	     rhsvl = atoi(val);
	     return mprog_veval(lhsvl, opr, rhsvl);
	     }
	  else
	  ...
	  case 'p': if (v_obj)
	     {
	     lhsvl = obj_index[v_obj->item_number].vnum;
	     rhsvl = atoi(val);
	     return mprog_veval(lhsvl, opr, rhsvl);
	     }
	  else
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'number'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   if (!str_cmp(buf, "name"))
      ...
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	 {
	  ...
	  case 'p': if (v_obj)
	     ...
	     return -1;
	  default:
	     bug ("Mob: %ld bad argument to 'name'", mob_index[mob->nr].vnum); 
	     return -1;
	 }
      }
   ...
   */
   bug ("Mob: %ld unknown ifchck", mob_index[mob->nr].vnum); 
   return -1;
  ...
  /* check for trueness of the ifcheck */
   ...
   while(loopdone == FALSE) /*scan over any existing or statements */
	 ...
	 cmnd++;
      if (*cmnd == '\0')
	 {
 	 bug ("Mob: %ld no commands after IF/OR", mob_index[mob->nr].vnum); 
 	 return null;
	 }
   ...
   if (flag)
	 ...
	 if (!str_cmp(buf, "else")) 
	    ...
	    while (str_cmp(buf, "endif")) 
		  ...
		  cmnd++;
 	       if (*cmnd == '\0')
		  {
		  bug ("Mob: %ld missing endif after else",
		       mob_index[mob->nr].vnum);
		  return null;
		  }
 	       morebuf = one_argument(cmnd,buf);
	 ...
	 if (*cmnd == '\0')
	    {
            bug ("Mob: %ld missing else or endif", mob_index[mob->nr].vnum); 
            return null;
	    }
	 morebuf = one_argument(cmnd, buf);
	 }
      ...
      while ((str_cmp(buf, "else")) && (str_cmp(buf,"endif")))
	 {
 	 ...
 	 if (*cmnd == '\0')
	    {
	    bug ("Mob: %ld missing an else or endif",
		 mob_index[mob->nr].vnum); 
	    return null;
	    }
 	 morebuf = one_argument(cmnd, buf);
	 ...
	 cmnd++;
      if (*cmnd == '\0')
	 { 
 	 bug ("Mob: %ld missing endif", mob_index[mob->nr].vnum); 
 	 return null;
	 }
      morebuf = one_argument(cmnd, buf);
      ...
      for (; ;) /*process the post-else commands until an endif is found.*/
	 {
 	 ...
 	 if (!str_cmp(buf, "else")) 
	    {
	    bug ("Mob: %ld found else in an else section",
		 mob_index[mob->nr].vnum); 
	    return null;
	    }
 	 if (!str_cmp(buf, "break"))
 	 ...
 	 if (*cmnd == '\0')
	    {
	    bug ("Mob:%ld missing endif in else section",
		 mob_index[mob->nr].vnum); 
	    return null;
	    }
 	 morebuf = one_argument(cmnd, buf);
                     ...
                     struct obj_data *obj, void *vo, struct char_data *rndm)
{
   ...
   *t = '\0';
   switch (ch) 
    ...
    default:
       bug("Mob: %ld bad $var", mob_index[mob->nr].vnum);
       bug("Mob: var: $%c",ch);
       break;
   }
 		   ...
 		   struct obj_data *obj, void *vo)
{
  ...
  /* get a random visable mortal player who is in the room with the mob */
   for (vch = world[IN_ROOM(mob)].people; vch; vch = vch->next_in_room)
      if (!IS_NPC(vch)
...
void mprog_greet_trigger(struct char_data *ch)
{
   ...
   for (vmob = world[IN_ROOM(ch)].people; vmob != NULL; vmob = vmob->next_in_room)
      if (IS_NPC(vmob)
	  && ch != vmob
	  && CAN_SEE(vmob, ch)
...
void mprog_speech_trigger(char *txt, struct char_data *mob)
{
   ...
   for (vmob = world[IN_ROOM(mob)].people; vmob != NULL; vmob = vmob->next_in_room)
