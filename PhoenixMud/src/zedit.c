/************************************************************************
 *  OasisOLC - zedit.c                                         v1.5     *
 *                                                                      * 
 *  Copyright 1996 Harvey Gilpin.                                       *
 ***********************************************************************/
 

#include "../localHeader/conf.h" 
#include "../localHeader/sysdep.h" 
#include "structs.h" 
#include "comm.h" 
#include "buffer.h" 
#include "utils.h" 
#include "db.h" 
#include "olc.h" 
#include "dg_scripts.h"
#include "constants.h"
/*
 * Turn on zedit debugging.  Please mail log results to greerga@van.ml.org
 * This will attempt to find the problem with replacing other zedit commands
 * when you add unrelated ones.
 */
#if 0
#define DEBUG  1
#endif

/*-------------------------------------------------------------------*/ 
/*. external data areas .*/ 
extern struct zone_data *zone_table;		  /*. db.c .*/ 
extern struct room_data *world;			  /*. db.c .*/ 
extern int top_of_zone_table;			  /*. db.c .*/ 
extern struct char_data *mob_proto;		  /*. db.c .*/ 
extern struct index_data *mob_index;		  /*. db.c .*/ 
extern struct obj_data *obj_proto;		  /*. db.c .*/ 
extern struct index_data *obj_index;		  /*. db.c .*/ 
extern char *reset_string[]; 
extern struct index_data **trig_index;
/*-------------------------------------------------------------------*/ 
/* function protos */ 
void zedit_disp_menu(struct descriptor_data *d); 
void zedit_setup(struct descriptor_data *d, int rroom_num); 
void add_cmd_to_list(struct reset_com **list, struct reset_com *newcmd, int pos); 
void remove_cmd_from_list(struct reset_com **list, int pos); 
void delete_command(struct descriptor_data *d, int pos); 
int new_command(struct descriptor_data *d, int pos); 
int start_change_command(struct descriptor_data *d, int pos); 
void zedit_disp_comtype(struct descriptor_data *d); 
void zedit_disp_arg1(struct descriptor_data *d); 
void zedit_disp_arg2(struct descriptor_data *d); 
void zedit_disp_arg3(struct descriptor_data *d); 
void zedit_disp_arg4(struct descriptor_data *d);
void zedit_save_internally(struct descriptor_data *d); 
void zedit_save_to_disk(int zone_num); 
void zedit_create_index(int znum, char *type); 
void zedit_new_zone(struct char_data *ch, int vzone_num); 
void zedit_disp_zone_flags(struct descriptor_data *d);
void zedit_disp_zone_status(struct descriptor_data *d);
void zedit_disp_zone_source(struct descriptor_data *d);
void zedit_disp_zone_continent(struct descriptor_data *d);
 
/*-------------------------------------------------------------------*/ 
/*. Nasty internal macros to clean up the code .*/ 
 
#define ZCMD zone_table[OLC_ZNUM(d)].cmd[subcmd] 
#define MYCMD OLC_ZONE(d)->cmd[subcmd] 
#define OLC_CMD(d)   (OLC_ZONE(d)->cmd[OLC_VAL(d)]) 
 
/*-------------------------------------------------------------------*\

  utility functions  
\*-------------------------------------------------------------------*/ 
 
void zedit_setup(struct  descriptor_data *d, int rroom_num) 
{ 
   struct zone_data *zone; 
   int subcmd = 0, count = 0, cmd_room = -1; 
 
  /*. Alloc some zone shaped space .*/ 
   CREATE(zone, struct zone_data, 1); 
 
  /*. Copy in zone header info .*/ 
   zone->name = str_dup(zone_table[OLC_ZNUM(d)].name); 
   zone->author = str_dup(zone_table[OLC_ZNUM(d)].author); 
   zone->editor = str_dup(zone_table[OLC_ZNUM(d)].editor); 
   zone->levels = str_dup(zone_table[OLC_ZNUM(d)].levels); 
   zone->lifespan = zone_table[OLC_ZNUM(d)].lifespan; 
   zone->top = zone_table[OLC_ZNUM(d)].top; 
   zone->reset_mode = zone_table[OLC_ZNUM(d)].reset_mode; 
   zone->bitvector = zone_table[OLC_ZNUM(d)].bitvector;
   zone->status = zone_table[OLC_ZNUM(d)].status; 
   zone->source = zone_table[OLC_ZNUM(d)].source; 
   zone->continent = zone_table[OLC_ZNUM(d)].continent; 
   zone->dateStarted = zone_table[OLC_ZNUM(d)].dateStarted; 
   zone->dateImped = zone_table[OLC_ZNUM(d)].dateImped; 
   zone->dateLastMod = zone_table[OLC_ZNUM(d)].dateLastMod; 
   zone->nameLastMod = str_dup(zone_table[OLC_ZNUM(d)].nameLastMod);
   zone->comment = str_dup(zone_table[OLC_ZNUM(d)].comment);
   zone->worldProof = str_dup(zone_table[OLC_ZNUM(d)].worldProof);
   zone->trigProof = str_dup(zone_table[OLC_ZNUM(d)].trigProof);
   zone->objBalanced = str_dup(zone_table[OLC_ZNUM(d)].objBalanced);

  /*. The remaining fields are used as a 'has been modified' flag .*/ 
   zone->number = 0;    /*. Header info has changed .*/ 
   zone->age = 0; /*. Commands have changed   .*/ 
   
  /*. Start the reset command list with a terminator .*/ 
   CREATE(zone->cmd, struct reset_com, 1); 
   zone->cmd[0].command = 'S'; 
 
  /*. Add all entried in zone_table that relate to this room .*/ 
   while(ZCMD.command != 'S') 
      { 
      switch(ZCMD.command) 
	 { 
	  case 'M': 
	  case 'O': 
	     cmd_room = ZCMD.arg3; 
	     break; 
	  case 'D': 
	  case 'R': 
	     cmd_room = ZCMD.arg1; 
	     break; 
	  case 'V':
	     if(ZCMD.arg1==WLD_TRIGGER)
		cmd_room=real_room(ZCMD.arg3);
	     break;
	  default: 
	     break; 
	 } 
      if(cmd_room == rroom_num) 
	 { 
#if defined(DEBUG)
	 log("zedit_setup called add_cmd_to_list.");
#endif
	 add_cmd_to_list(&(zone->cmd), &ZCMD, count); 
	 count++; 
	 } 
      subcmd++; 
      } 
 
   OLC_ZONE(d) = zone; 
  /*. Display main menu .*/ 
   zedit_disp_menu(d); 
} 
 
 
/*-------------------------------------------------------------------*/ 
/*. Create a new zone .*/ 
 
void zedit_new_zone(struct char_data *ch, int vzone_num) 
{ 
   FILE *fp; 
   struct zone_data *new_table; 
   int i, room;
   char *buf;
 
   if (vzone_num < 0) 
      {
      send_to_char(ch, "You can't make negative zones.\r\n");
      return;
      }
   else if(vzone_num > 3260) 
      { 
      send_to_char(ch, "3260 is the highest zone allowed.\r\n"); 
      return; 
      } 
 
  /*. Check zone does not exist .*/ 
   room = vzone_num * 100; 
   for (i = 0; i <= top_of_zone_table; i++) 
      if ((zone_table[i].number * 100 <= room) && (zone_table[i].top >= room)) 
	 { 
	 send_to_char(ch, "A zone already covers that area.\r\n"); 
	 return; 
	 } 
 
  /*. Create Zone file .*/ 
   buf=get_buffer(256);
   sprintf(buf, "%s/%i.zon", ZON_PREFIX, vzone_num); 
   if(!(fp = fopen(buf, "w"))) 
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new zone file"); 
      send_to_char(ch, "Could not write zone file.\r\n");
      release_buffer(buf);
      return; 
      } 
   fprintf(fp,  
	   "#%d\n" 
	   "New Zone~\n" 
	   "%d 30 2\n" 
	   "0 0\n"
	   "S\n" 
	   "$\n",  
	   vzone_num, 
	   (vzone_num * 100) + 99 
      ); 
   fclose(fp); 
 
  /*. Create Rooms file .*/ 
   sprintf(buf, "%s/%d.wld", WLD_PREFIX, vzone_num); 
   if(!(fp = fopen(buf, "w")))  
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new world file"); 
      send_to_char(ch, "Could not write world file.\r\n");
      release_buffer(buf);
      return; 
      } 
   fprintf(fp,  
	   "#%d\n" 
	   "The Begining~\n" 
	   "Not much here.\n" 
	   "~\n" 
	   "%d 0 0\n" 
	   "S\n" 
	   "$\n", 
	   vzone_num * 100, 
	   vzone_num 
      ); 
   fclose(fp); 
 
  /*. Create Mobiles file .*/ 
   sprintf(buf, "%s/%i.mob", MOB_PREFIX, vzone_num); 
   if(!(fp = fopen(buf, "w"))) 
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new mob file"); 
      send_to_char(ch, "Could not write mobile file.\r\n");
      release_buffer(buf);
      return; 
      } 
   fprintf(fp, "$\n"); 
   fclose(fp); 
 
  /*. Create Objects file .*/ 
   sprintf(buf, "%s/%i.obj", OBJ_PREFIX, vzone_num); 
   if(!(fp = fopen(buf, "w")))  
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new obj file"); 
      send_to_char(ch, "Could not write object file.\r\n");
      release_buffer(buf);
      return; 
      } 
   fprintf(fp, "$\n"); 
   fclose(fp); 
 
  /*. Create Shops file .*/ 
   sprintf(buf, "%s/%i.shp", SHP_PREFIX, vzone_num); 
   if(!(fp = fopen(buf, "w"))) 
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new shop file"); 
      send_to_char(ch, "Could not write shop file.\r\n");
      release_buffer(buf);
      return; 
      } 
   fprintf(fp, "$~\n"); 
   fclose(fp); 
 
  /*. Create Gld file . */
   sprintf(buf, "%s/%i.gld", GLD_PREFIX, vzone_num);
   if (!(fp = fopen(buf, "w"))) 
      {
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new GM file");
      return;
      }
   fprintf(fp, "$~\n");
   fclose(fp);

  /*
   * Create the trigger file.
   */
   sprintf(buf, "%s/%d.trg", TRG_PREFIX, vzone_num);
   if (!(fp = fopen(buf, "w"))) 
      {
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Can't write new trigger file");
      send_to_char(ch, "Could not write trigger file.\r\n");
      return;
      }
   fprintf(fp, "$~\n");
   fclose(fp);
   
   
  /*. Update index files .*/ 
   zedit_create_index(vzone_num, "zon"); 
   zedit_create_index(vzone_num, "wld"); 
   zedit_create_index(vzone_num, "mob"); 
   zedit_create_index(vzone_num, "obj"); 
   zedit_create_index(vzone_num, "shp"); 
   zedit_create_index(vzone_num, "gld"); 
   zedit_create_index(vzone_num, "trg");

  /*
   * Make a new zone in memory. This was the source of all the zedit new
   * crashes reported to the CircleMUD list. It was happily overwriting
   * the stack.  This new loop by Andrew Helm fixes that problem and is
   * more understandable at the same time.
   *
   * The variable is 'top_of_zone_table_table + 2' because we need record 0
   * through top_of_zone (top_of_zone_table + 1 items) and a new one which
   * makes it top_of_zone_table + 2 elements large.
   */
   CREATE(new_table, struct zone_data, top_of_zone_table+2); 
   new_table[top_of_zone_table + 1].number = 320000; 
   if (vzone_num > zone_table[top_of_zone_table].number)
      {
     /*
      * We're adding to the end of the zone table, copy all of the current
      * top_of_zone_table + 1 items over and set write point to before the
      * the last record for the for() loop below.
      */
      memcpy(new_table, zone_table, (sizeof (struct zone_data) * (top_of_zone_table + 1)));
      i = top_of_zone_table + 1;
      }
   else
     /*
      * Copy over all the zones that are before this zone.
      */
      for (i = 0; vzone_num > zone_table[i].number; i++)
	 new_table[i] = zone_table[i];
   
  /*
   * Ok, insert the new zone here.
   */  
   new_table[i].name = str_dup("New Zone");
   new_table[i].author = str_dup("Enter Name Here");
   new_table[i].editor = str_dup("Enter Name Here");
   new_table[i].levels = str_dup("Enter Zone Levels Here");
   new_table[i].number = vzone_num;
   new_table[i].top = (vzone_num * 100) + 99;
   new_table[i].lifespan = 30;
   new_table[i].age = 0;
   new_table[i].reset_mode = 2;
   new_table[i].bitvector = 0;
   new_table[i].source = 0;
   new_table[i].status = 0;
   new_table[i].continent = 0;
   new_table[i].dateStarted = time(0);
   new_table[i].dateImped = 0;
   new_table[i].dateLastMod = 0;
   new_table[i].nameLastMod = str_dup("Will be set on first edit");
   new_table[i].comment = str_dup("None");
   new_table[i].worldProof = str_dup("None");
   new_table[i].trigProof = str_dup("None");
   new_table[i].objBalanced = str_dup("None");
   
/*    new_table[i].source = ZONE_ORIGINAL; */
/*    new_table[i].status = ZONE_UNFINISHED; */

  /*
   * No zone commands, just terminate it with an 'S'
   */
   CREATE(new_table[i].cmd, struct reset_com, 1);
   new_table[i].cmd[0].command = 'S';
   
  /*
   * Copy remaining zones into the table one higher, unless of course we
   * are appending to the end in which case this loop will not be used.
   */
   for (; i <= top_of_zone_table; i++)
      new_table[i + 1] = zone_table[i];
   
  /*
   * Look Ma, no memory leak!
   */
   free(zone_table); 
   zone_table = new_table; 
   top_of_zone_table++; 
 
   mudlogf(BRF, MAX(LVL_BUILDER,GET_INVIS_LEV(ch)), TRUE,
	  "OLC: %s creates new zone #%d", GET_NAME(ch), vzone_num); 
   send_to_char(ch, "Zone created successfully.\r\n"); 
   release_buffer(buf);
   return; 
} 
 
/*-------------------------------------------------------------------*/ 
 
void zedit_create_index(int znum, char *type) 
{ 
   FILE *newfile, *oldfile; 
   char *new_name;
   char *old_name;
   char *prefix, *buf,*buf1; 
   int num, found = FALSE; 
 
   switch(*type) 
      { 
       case 'z': 
	  prefix = ZON_PREFIX; 
	  break; 
       case 'w': 
	  prefix = WLD_PREFIX; 
	  break; 
       case 'o': 
	  prefix = OBJ_PREFIX; 
	  break; 
       case 'm': 
	  prefix = MOB_PREFIX; 
	  break; 
       case 's': 
	  prefix = SHP_PREFIX; 
	  break; 
       case 'g': 
	  prefix = GLD_PREFIX; 
	  break; 
       case 't':
	  prefix = TRG_PREFIX;
	  break;
       default: 
	 /*. Caller messed up .*/ 
	  return; 
      } 
   old_name=get_buffer(64);
   new_name=get_buffer(64);
   sprintf(old_name, "%s/index", prefix); 
   sprintf(new_name, "%s/newindex", prefix); 
 
   if(!(oldfile = fopen(old_name, "r"))) 
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Failed to open %s", old_name);  
      release_buffer(new_name);
      release_buffer(old_name);
      return; 
      } 
   else if(!(newfile = fopen(new_name, "w"))) 
      { 
      mudlogf(BRF, LVL_IMPL, TRUE,"SYSERR: OLC: Failed to open %s", new_name);  
      release_buffer(new_name);
      release_buffer(old_name);
      return; 
      } 
   
  /*
   * Index contents must be in order: search through the old file for 
   * the right place, insert the new file, then copy the rest over. 
   */ 
   buf=get_buffer(512);
   buf1=get_buffer(512);

   sprintf(buf1, "%d.%s", znum, type); 
   while(get_line(oldfile, buf)) 
      { 
      if (*buf == '$') 
	 { 
	 if (!found) 
	    fprintf(newfile, "%s\n", buf1); 
	 fprintf(newfile, "$\n"); 
	 break; 
	 } 
      else if (!found) 
	 { 
	 sscanf(buf, "%d", &num); 
	 if(num == znum)
	    found = TRUE;
	 else if (num > znum) 
	    { 
	    found = TRUE; 
	    fprintf(newfile, "%s\n", buf1); 
	    } 
	 } 
      fprintf(newfile, "%s\n", buf); 
      } 
     
   fclose(newfile); 
   fclose(oldfile); 
  /*
   * Out with the old, in with the new 
   */ 
   remove(old_name); 
   rename(new_name, old_name); 
   release_buffer(buf1);
   release_buffer(buf);
   release_buffer(new_name);
   release_buffer(old_name);
} 
 
/*-------------------------------------------------------------------*/ 
/* 
 * Save all the information on the players descriptor back into the zone table
 */ 
 
void zedit_save_internally(struct descriptor_data *d) 
{ 
   int subcmd = 0, cmd_room = -2, rroom_num; 
 
   rroom_num = real_room(OLC_NUM(d)); 
 
  /*. Zap all entried in zone_table that relate to this room .*/ 
   while(ZCMD.command != 'S') 
      { 
      switch(ZCMD.command) 
	 { 
	  case 'M': 
	  case 'O': 
	     cmd_room = ZCMD.arg3; 
	     break; 
	  case 'D': 
	  case 'R': 
	     cmd_room = ZCMD.arg1; 
	     break; 
	  default: 
	     break; 
	 } 
      if(cmd_room == rroom_num) 
	 {
#if defined(DEBUG)
	 log("zedit_save_internally called remove_cmd_from_list.");
#endif
	 remove_cmd_from_list(&(zone_table[OLC_ZNUM(d)].cmd), subcmd); 
	 }
      else 
	 subcmd++; 
      } 
 
  /*. Now add all the entries in the players descriptor list .*/ 
   subcmd = 0; 
   while(MYCMD.command != 'S') 
      { 
#if defined(DEBUG)
      log("zedit_save_internally called add_cmd_to_list.");
#endif
      add_cmd_to_list(&(zone_table[OLC_ZNUM(d)].cmd), &MYCMD, subcmd); 
      subcmd++; 
      } 
 
  /*. Finally, if zone headers have been changed, copy over .*/ 
   if (OLC_ZONE(d)->number) 
      { 
      free(zone_table[OLC_ZNUM(d)].name); 
      zone_table[OLC_ZNUM(d)].name  = str_dup(OLC_ZONE(d)->name); 
      free(zone_table[OLC_ZNUM(d)].author); 
      zone_table[OLC_ZNUM(d)].author  = str_dup(OLC_ZONE(d)->author); 
      free(zone_table[OLC_ZNUM(d)].editor); 
      zone_table[OLC_ZNUM(d)].editor  = str_dup(OLC_ZONE(d)->editor); 
      free(zone_table[OLC_ZNUM(d)].levels); 
      zone_table[OLC_ZNUM(d)].levels  = str_dup(OLC_ZONE(d)->levels);  
      zone_table[OLC_ZNUM(d)].top  = OLC_ZONE(d)->top; 
      zone_table[OLC_ZNUM(d)].status  = OLC_ZONE(d)->status; 
      zone_table[OLC_ZNUM(d)].source  = OLC_ZONE(d)->source; 
      zone_table[OLC_ZNUM(d)].continent  = OLC_ZONE(d)->continent; 
      zone_table[OLC_ZNUM(d)].reset_mode  = OLC_ZONE(d)->reset_mode; 
      zone_table[OLC_ZNUM(d)].lifespan  = OLC_ZONE(d)->lifespan; 
      zone_table[OLC_ZNUM(d)].bitvector  = OLC_ZONE(d)->bitvector; 
      zone_table[OLC_ZNUM(d)].dateStarted  = OLC_ZONE(d)->dateStarted; 
      zone_table[OLC_ZNUM(d)].dateImped  = OLC_ZONE(d)->dateImped; 
      zone_table[OLC_ZNUM(d)].dateLastMod  = OLC_ZONE(d)->dateLastMod; 
      free(zone_table[OLC_ZNUM(d)].nameLastMod);
      zone_table[OLC_ZNUM(d)].nameLastMod  = str_dup(OLC_ZONE(d)->nameLastMod);
      free(zone_table[OLC_ZNUM(d)].comment);
      zone_table[OLC_ZNUM(d)].comment  = str_dup(OLC_ZONE(d)->comment);
      free(zone_table[OLC_ZNUM(d)].worldProof); 
      zone_table[OLC_ZNUM(d)].worldProof  = str_dup(OLC_ZONE(d)->worldProof); 
      free(zone_table[OLC_ZNUM(d)].trigProof); 
      zone_table[OLC_ZNUM(d)].trigProof  = str_dup(OLC_ZONE(d)->trigProof); 
      free(zone_table[OLC_ZNUM(d)].objBalanced); 
      zone_table[OLC_ZNUM(d)].objBalanced  = str_dup(OLC_ZONE(d)->objBalanced); 
      } 
   olc_add_to_save_list(zone_table[OLC_ZNUM(d)].number, OLC_SAVE_ZONE); 
} 
 
 
/*-------------------------------------------------------------------*/ 
/*. Save all the zone_table for this zone to disk.  Yes, this could 
  automatically comment what it saves out, but why bother when you 
  have an OLC as cool as this ? :> .*/ 
 
#undef ZCMD
#define ZCMD   (zone_table[zone_num].cmd[subcmd])
void zedit_save_to_disk(int zone_num)
{ 
   int subcmd, arg1 = -1, arg2 = -1, arg3 = -1, arg4 = -1; 
   char *fname=get_buffer(64); 
   char *buf=get_buffer(MAX_STRING_LENGTH);
   char *buf1=get_buffer(MAX_STRING_LENGTH);
   long zone_flag;
   const char *comment=NULL;
   FILE *zfile; 
 
   sprintf(fname, "%s/%ld.new", ZON_PREFIX,  
           zone_table[zone_num].number); 
 
   if(!(zfile = fopen(fname, "w"))) 
      { 
      mudlogf(BRF, LVL_BUILDER, TRUE,
	     "SYSERR: OLC: zedit_save_to_disk:  Can't write zone %ld.",  
	      zone_table[zone_num].number);  
      release_buffer(buf1);
      release_buffer(buf);
      release_buffer(fname);
      return; 
      } 
   fprintf(zfile,"@Version: %d\n",CUR_ZONE_VER);
   
   zone_flag=zone_table[zone_num].bitvector;
   REMOVE_BIT(zone_flag,Z_IDLE|Z_QUEUED);
   flagascii_conv(buf1,zone_flag);
  /*. Print zone header to file .*/ 
   fprintf(zfile,
	   "#%ld\n" 
	   "%s~\n" 
	   "%s~\n" 
	   "%s~\n" 
	   "%s~\n"
	   "%ld %ld %ld\n"
	   "%s~\n"
	   "%s~\n"
      "%s~\n"
      "%s~\n"
      "%s~\n"
	   "%ld %d %d %d %d %d\n"
	   "0 %s\n", 
	   zone_table[zone_num].number, 
	   (zone_table[zone_num].name&& *zone_table[zone_num].name) ? 
	   zone_table[zone_num].name : "undefined", 
	   (zone_table[zone_num].author&& *zone_table[zone_num].author) ? 
	   zone_table[zone_num].author : "undefined", 
	   (zone_table[zone_num].editor&& *zone_table[zone_num].editor) ? 
	   zone_table[zone_num].editor : "undefined", 
	   (zone_table[zone_num].levels&& *zone_table[zone_num].levels) ? 
	   zone_table[zone_num].levels : "undefined", 
	   zone_table[zone_num].dateStarted, 
	   zone_table[zone_num].dateImped, 
	   zone_table[zone_num].dateLastMod, 
	   (zone_table[zone_num].nameLastMod&& *zone_table[zone_num].nameLastMod) ? 
	   zone_table[zone_num].nameLastMod : "undefined", 
	   (zone_table[zone_num].comment&& *zone_table[zone_num].comment) ? 
	   zone_table[zone_num].comment : "undefined", 
      (zone_table[zone_num].worldProof&& *zone_table[zone_num].worldProof) ? 
      zone_table[zone_num].worldProof : "undefined", 
      (zone_table[zone_num].trigProof&& *zone_table[zone_num].trigProof) ? 
      zone_table[zone_num].trigProof : "undefined", 
      (zone_table[zone_num].objBalanced&& *zone_table[zone_num].objBalanced) ? 
      zone_table[zone_num].objBalanced : "undefined", 
	   zone_table[zone_num].top, 
	   zone_table[zone_num].lifespan,  
	   zone_table[zone_num].reset_mode,
	   (int)zone_table[zone_num].status,
	   (int)zone_table[zone_num].source,
	   (int)zone_table[zone_num].continent,
	   buf1
      ); 
   release_buffer(buf1);

#if defined(ZEDIT_HELP_IN_FILE)
   fprintf(zfile,
	   "*             arg1       arg2      arg3            arg4\n"
	   "* M (Mobile)  Mob-Vnum   Wld-Max   Room-Vnum         --  \n"
	   "* O (Object)  Obj-Vnum   Wld-Max   Room-Vnum       %%load\n"
	   "* G (Give)    Obj-Vnum   Wld-Max   %%load            --  \n"
	   "* E (Equip)   Obj-Vnum   Wld-Max   EQ-Position     %%load\n"
	   "* P (Put)     Obj-Vnum   Wld-Max   Target-Obj-Vnum %%load\n"
	   "* D (Door)    Room-Vnum  Door-Dir  Door-State        --  \n"
	   "* R (Remove)  Room-Vnum  Obj-Vnum    --              --  \n"
	   "* T (Trigger) Trig-Type  Trig-Vnum Unused\n"
	   "* V (Variable)Trig-Type  Context   Room#  Varname Value\n"
	   "* F (Force)   String Arg\n"
      );
#endif
 
   for(subcmd = 0; ZCMD.command != 'S'; subcmd++) 
      { 
      arg1 = 0; 
      arg2 = 0; 
      arg3 = 0; 
      arg4 = 0; 
      switch (ZCMD.command) 
	 { 
	  case 'M': 
	     arg1 = mob_index[ZCMD.arg1].vnum; 
	     arg2 = ZCMD.arg2; 
	     arg3 = GET_ROOM_VNUM(ZCMD.arg3);
	     arg4 = -1;
	     comment = mob_proto[ZCMD.arg1].player.short_descr;
	     break; 
	  case 'O': 
	     arg1 = obj_index[ZCMD.arg1].vnum; 
	     arg2 = ZCMD.arg2; 
	     arg3 = GET_ROOM_VNUM(ZCMD.arg3);
	     arg4 = ZCMD.arg4;
	     comment = obj_proto[ZCMD.arg1].short_description;
	     break; 
	  case 'G': 
	     ZCMD.if_flag=1;
	     arg1 = obj_index[ZCMD.arg1].vnum; 
	     arg2 = ZCMD.arg2; 
	     arg3 = ZCMD.arg3; 
	     arg4 = -1;
	     comment = obj_proto[ZCMD.arg1].short_description;
	     break; 
	  case 'E': 
	     ZCMD.if_flag=1;
	     arg1 = obj_index[ZCMD.arg1].vnum; 
	     arg2 = ZCMD.arg2; 
	     arg3 = ZCMD.arg3; 
	     arg4 = ZCMD.arg4;
	     comment = obj_proto[ZCMD.arg1].short_description;
	     break; 
	  case 'P': 
	     ZCMD.if_flag=1;
	     arg1 = obj_index[ZCMD.arg1].vnum; 
	     arg2 = ZCMD.arg2; 
	     arg3 = obj_index[ZCMD.arg3].vnum; 
	     arg4 = ZCMD.arg4;
	     comment = obj_proto[ZCMD.arg1].short_description;
	     break; 
	  case 'D': 
	     arg1 = GET_ROOM_VNUM(ZCMD.arg1); 
	     arg2 = ZCMD.arg2; 
	     arg3 = ZCMD.arg3; 
	     arg4 = -1;
	     comment = world[ZCMD.arg1].name;
	     break; 
	  case 'R': 
	     arg1 = GET_ROOM_VNUM(ZCMD.arg1); 
	     arg2 = obj_index[ZCMD.arg2].vnum; 
	     arg3 = -1; 
	     arg4 = -1;
	     comment = obj_proto[ZCMD.arg2].short_description;
	     break; 
	  case 'F':
	     fprintf(zfile,"F 1 %s\n",ZCMD.sarg);
	     break;
	  case 'Z': 
	     ZCMD.if_flag=0;
	     arg1 = 0; 
	     arg2 = 0; 
	     arg3 = 0; 
	     arg4 = 0; 
	     comment = "Maze Zone";
	     break; 
	  case 'T':
	     arg1 = ZCMD.arg1; /* trigger type */
	     arg2 = ZCMD.arg2;
	     break;
	  case 'V':
	     arg1 = ZCMD.arg1; /* trigger type */
	     arg2 = ZCMD.arg2; /* context */
	     break;

	  case '*': 
	    /*. Invalid commands are replaced with '*' - Ignore them .*/ 
	     continue; 
	  default: 
	     mudlogf(BRF, LVL_BUILDER, TRUE, "SYSERR: OLC: z_save_to_disk(): Unknown cmd '%c' - NOT saving", ZCMD.command);  
	     continue; 
	 } 
      if(ZCMD.command=='V')
	 fprintf(zfile, "%c %d %d %d %d %s %s\n",
		 ZCMD.command, ZCMD.if_flag, arg1, arg2, arg3, ZCMD.sarg,
		 ZCMD.sarg2);
      else if(ZCMD.command!='F')
	 fprintf(zfile, "%c %d %d %d %d %d \t(%s)\n", 
		 ZCMD.command, ZCMD.if_flag, arg1, arg2, arg3,arg4,comment); 
      } 
   fprintf(zfile, "S\n$\n"); 
   fclose(zfile); 
   sprintf(buf, "%s/%ld.zon", ZON_PREFIX, zone_table[zone_num].number);
  /*
   * We're fubar'd if we crash between the two lines below.
   */
   remove(buf);
   rename(fname, buf);
   
   olc_remove_from_save_list(zone_table[zone_num].number, OLC_SAVE_ZONE); 
   release_buffer(buf);
   release_buffer(fname);
} 
 
/*-------------------------------------------------------------------*/ 
/*
 * Some common code to count the number of comands in the list.
 */
int count_commands(struct reset_com *list)
{
   int count = 0;

   while (list[count].command != 'S')
      count++;
   
   return count;
}

/*-------------------------------------------------------------------*/ 
/*. Adds a new reset command into a list.  Takes a pointer to the list 
  so that it may play with the memory locations.*/ 
 
void add_cmd_to_list(struct reset_com **list, struct reset_com *newcmd, int pos) 
{  
   int count = 0, i, l; 
   struct reset_com *newlist; 
  /*
   * Count number of commands (not including terminator).
   */
   count = count_commands(*list);
   
  /*
   * Value is +2 for the terminator and new field to add.
   */
   CREATE(newlist, struct reset_com, count + 2);
   
  /*
   * Even tighter loop to copy the old list and insert a new command.
   */
   for (i = 0, l = 0; i <= count; i++) 
      {
      newlist[i] = ((i == pos) ? *newcmd : (*list)[l++]);
#if defined(DEBUG)
      log("add_cmd_to_list: added %c %d %d %d %d",
	      newlist[i].command, newlist[i].arg1, newlist[i].arg2,
	      newlist[i].arg3, newlist[i].line);
#endif
      }
  /*
   * Add terminator, then insert new list.
   */
   newlist[count + 1].command = 'S';
   free(*list);
   *list = newlist;
}
 
/*-------------------------------------------------------------------*/ 
/*. Remove a reset command from a list.  Takes a pointer to the list 
  so that it may play with the memory locations.*/ 
 
void remove_cmd_from_list(struct reset_com **list, int pos) 
{  
   int count = 0, i, l; 
   struct reset_com *newlist; 
   
  /*
   * Count number of commands (not including terminator)  
   */
   count = count_commands(*list);
   
  /*
   * Value is 'count' because we didn't include the terminator above
   * but since we're deleting one thing anyway we want one less.
   */
   CREATE(newlist, struct reset_com, count);
   
  /*
   * Even tighter loop to copy old list and skip unwanted command.
   */
   for (i = 0, l = 0; i < count; i++) 
      {
      if (i != pos) 
	 {
#if defined(DEBUG)
	 log("remove_cmd_from_list: kept %c %d %d %d %d",
	     (*list)[i].command, (*list)[i].arg1, (*list)[i].arg2,
	     (*list)[i].arg3, (*list)[i].line);
#endif
	 newlist[l++] = (*list)[i];
	 }
#if defined(DEBUG)
      else
	 log("remove_cmd_from_list: deleted %c %d %d %d %d",
	     (*list)[i].command, (*list)[i].arg1, (*list)[i].arg2,
	     (*list)[i].arg3, (*list)[i].line);
      
#endif
      }
  /*
   * Add the terminator, then insert the new list.
   */
   newlist[count - 1].command = 'S';
   free(*list);
   *list = newlist;
} 
 
/*-------------------------------------------------------------------*/ 
/*. Error check user input and then add new (blank) command .*/ 
 
int new_command(struct descriptor_data *d, int pos) 
{  
   int subcmd = 0; 
   struct reset_com *new_com; 
 
  /*
   * Error check to ensure users hasn't given too large an index  
   */
   while (MYCMD.command != 'S')
      subcmd++;
   
   if ((pos > subcmd) || (pos < 0))
      return 0;
   
  /*
   * Ok, let's add a new (blank) command 
   */
   CREATE(new_com, struct reset_com, 1);
   new_com->command = 'N';
#if defined(DEBUG)
   log("new_command called add_cmd_to_list.");
#endif
   new_com->room_num=OLC_NUM(d);
   add_cmd_to_list(&OLC_ZONE(d)->cmd, new_com, pos);
   return 1;
}
 
 
/*-------------------------------------------------------------------*/ 
/*. Error check user input and then remove command .*/ 
 
void delete_command(struct descriptor_data *d, int pos) 
{  
   int subcmd = 0; 
 
  /*. Error check to ensure users hasn't given too large an index .*/ 
   while(MYCMD.command != 'S') 
      subcmd++; 
   
   if ((pos >= subcmd) || (pos < 0)) 
      return; 
     
  /*. Ok, let's zap it .*/ 
#if defined(DEBUG)
   log("delete_command called remove_cmd_from_list.");
#endif
   remove_cmd_from_list(&OLC_ZONE(d)->cmd, pos); 
} 
 
/*-------------------------------------------------------------------*/ 
/*. Error check user input and then setup change .*/ 
 
int start_change_command(struct descriptor_data *d, int pos) 
{  
   int subcmd = 0; 
 
  /*. Error check to ensure users hasn't given too large an index .*/ 
   while(MYCMD.command != 'S') 
      subcmd++; 
   
   if ((pos >= subcmd) || (pos < 0)) 
      return 0; 
     
  /*. Ok, let's get editing .*/ 
   OLC_VAL(d) = pos; 
   return 1; 
} 
 
/************************************************************************** 
 Menu functions  
 **************************************************************************/ 
 
/* the main menu */ 
void zedit_disp_menu(struct descriptor_data * d) 
{ 
   int subcmd = 0, room, counter = 0; 
   char *buf1=get_buffer(MAX_STRING_LENGTH);
   char *buf2=get_buffer(MAX_STRING_LENGTH);

   get_char_cols(d->character); 
   room = real_room(OLC_NUM(d)); 
 
   sprintbit(OLC_ZONE(d)->bitvector,zone_bits,buf1);
  /*. Menu header .*/ 
   send_to_char(d->character, 
	   "[H[J" 
	   "Room number: %s%d%s  Room zone: %s%ld\r\n" 
	   "%sZ%s) Zone name   : %s%s\r\n" 
	   "%sA%s) Zone author : %s%s\r\n" 
	   "%sB%s) Zone editor : %s%s\r\n" 
	   "%sV%s) Zone levels : %s%s\r\n" 
	   "%sL%s) Lifespan    : %s%d minutes\r\n" 
	   "%sT%s) Top of zone : %s%ld\r\n"
	   "%sC%s) Status      : %s%s\r\n"
	   "%sG%s) Source      : %s%s\r\n"
	   "%sH%s) Continent   : %s%s\r\n"
	   "%sR%s) Reset Mode  : %s%s%s\r\n" 
	   "%sF%s) Zone Flags  : %s%s%s\r\n"
	   "%sI%s) Comment     : %s%s%s\r\n"
      "%sJ%s) Wld Proofer : %s%s%s\r\n"
      "%sK%s) Trg Proofer : %s%s%s\r\n"
      "%sM%s) Obj Balancer: %s%s%s\r\n"
	   "[Command list]\r\n",
 
	   cyn, OLC_NUM(d), nrm, 
	   cyn, zone_table[OLC_ZNUM(d)].number, 
	   grn, nrm, yel, OLC_ZONE(d)->name ? OLC_ZONE(d)->name : "<NONE!>", 
	   grn, nrm, yel, OLC_ZONE(d)->author ? OLC_ZONE(d)->author :"<NONE!>",
	   grn, nrm, yel, OLC_ZONE(d)->editor ? OLC_ZONE(d)->editor :"<NONE!>",
	   grn, nrm, yel, OLC_ZONE(d)->levels ? OLC_ZONE(d)->levels :"<NONE!>",
	   grn, nrm, yel, OLC_ZONE(d)->lifespan, 
	   grn, nrm, yel, OLC_ZONE(d)->top, 
	   grn, nrm, yel, zone_status[(int)OLC_ZONE(d)->status], 
	   grn, nrm, yel, zone_source[(int)OLC_ZONE(d)->source], 
	   grn, nrm, yel, zone_continent[(int)OLC_ZONE(d)->continent][0], 
	   grn, nrm, yel, reset_string[OLC_ZONE(d)->reset_mode], nrm,
	   grn, nrm, yel, buf1, nrm,
	   grn, nrm,yel,OLC_ZONE(d)->comment?OLC_ZONE(d)->comment:"<NONE!>",nrm,
      grn, nrm, yel, OLC_ZONE(d)->worldProof ? OLC_ZONE(d)->worldProof :"<NONE!>",nrm,
      grn, nrm, yel, OLC_ZONE(d)->trigProof ? OLC_ZONE(d)->trigProof :"<NONE!>",nrm,
      grn, nrm, yel, OLC_ZONE(d)->objBalanced ? OLC_ZONE(d)->objBalanced :"<NONE!>",nrm
      ); 
 
  /*. Print the commands for this room into display buffer .*/ 
   while(MYCMD.command != 'S') 
      { 
      
/*. Translate what the command means .*/ 
      switch(MYCMD.command) 
	 { 
	  case 'M': 
	     sprintf(buf2, "%sLoad %s [%s%ld%s], Max : %ld (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     mob_proto[MYCMD.arg1].player.short_descr, 
		     cyn, mob_index[MYCMD.arg1].vnum, yel, 
		     MYCMD.arg2,MYCMD.room_num
		); 
	     break; 
	  case 'G': 
	     sprintf(buf2, "%sGive it %s [%s%ld%s], Chance : %ld%% (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     obj_proto[MYCMD.arg1].short_description, 
		     cyn, obj_index[MYCMD.arg1].vnum, yel, 
		     MYCMD.arg3?(101-MYCMD.arg3):100 ,MYCMD.room_num
		); 
	     break; 
	  case 'O': 
	     sprintf(buf2, "%sLoad %s [%s%ld%s], Chance : %ld%% (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     obj_proto[MYCMD.arg1].short_description, 
		     cyn, obj_index[MYCMD.arg1].vnum, yel, 
		     MYCMD.arg4?(101-MYCMD.arg4):100 ,MYCMD.room_num
		); 
	     break; 
	  case 'E': 
	     sprintf(buf2, "%sEquip with %s [%s%ld%s], %s, Chance : %ld%% (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     obj_proto[MYCMD.arg1].short_description, 
		     cyn, obj_index[MYCMD.arg1].vnum, yel, 
		     equipment_types[MYCMD.arg3], 
		     MYCMD.arg4?(101-MYCMD.arg4):100 ,MYCMD.room_num
		); 
	     break; 
	  case 'P': 
	     sprintf(buf2, "%sPut %s [%s%ld%s] in %s [%s%ld%s], Chance : %ld%% (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     obj_proto[MYCMD.arg1].short_description, 
		     cyn, obj_index[MYCMD.arg1].vnum, yel, 
		     obj_proto[MYCMD.arg3].short_description, 
		     cyn, obj_index[MYCMD.arg3].vnum, yel, 
		     MYCMD.arg4?(101-MYCMD.arg4):100 ,MYCMD.room_num
		); 
	     break; 
	  case 'R': 
	     sprintf(buf2, "%sRemove %s [%s%ld%s] from room. (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     obj_proto[MYCMD.arg2].short_description, 
		     cyn, obj_index[MYCMD.arg2].vnum, yel ,MYCMD.room_num
		); 
	     break; 
	  case 'D': 
	     sprintf(buf2, "%sSet door %s as %s. (%ld)", 
		     MYCMD.if_flag ? " then " : "", 
		     dirs[MYCMD.arg2], 
		     MYCMD.arg3 ? ((MYCMD.arg3 == 1) ?  
				   "closed" : "locked") : "open",MYCMD.room_num
		); 
	     break;
	  case 'T':
	     sprintf(buf2, "%sAttach trigger %s%s%s [%s%ld%s] to %s",
		     MYCMD.if_flag ? " then " : "",
		     cyn, trig_index[real_trigger(MYCMD.arg2)]->proto->name, yel, 
		     cyn, MYCMD.arg2, yel,
		     ((MYCMD.arg1 == MOB_TRIGGER) ? "mobile" :
		      ((MYCMD.arg1 == OBJ_TRIGGER) ? "object" :
		       ((MYCMD.arg1 == WLD_TRIGGER)? "room" : "????"))));
	     break;
	  case 'V':
	     sprintf(buf2, "%sAssign global %s:%ld to %s = %s",
		     MYCMD.if_flag ? " then " : "",
		     MYCMD.sarg, MYCMD.arg2,
		     ((MYCMD.arg1 == MOB_TRIGGER) ? "mobile" :
		      ((MYCMD.arg1 == OBJ_TRIGGER) ? "object" :
		       ((MYCMD.arg1 == WLD_TRIGGER)? "room" : "????"))),
		     MYCMD.sarg2);
	     break;
	  case 'F':
	     sprintf(buf2," then Force to %s",MYCMD.sarg);
	     break;
	  default: 
	     strcpy(buf2, "<Unknown Command>"); 
	     break; 
	 } 
     /*. Build the display buffer for this command .*/ 
      send_to_char(d->character, "%s%d - %s%s\r\n", 
	      nrm, counter++, yel, 
	      buf2 
	 ); 
      subcmd++; 
      } 
  /*. Finish off menu .*/ 
   send_to_char(d->character, 
	   "%s%d - <END OF LIST>\r\n" 
	   "%sN%s) New command.\r\n" 
	   "%sE%s) Edit a command.\r\n" 
	   "%sD%s) Delete a command.\r\n" 
	   "%sQ%s) Quit\r\nEnter your choice : ", 
	   nrm, counter, 
	   grn, nrm, grn, nrm, grn, nrm, grn, nrm 
      ); 
 
   OLC_MODE(d) = ZEDIT_MAIN_MENU; 
   release_buffer(buf2);
   release_buffer(buf1);
} 
 
 
/*-------------------------------------------------------------------*/ 
/*. Print the command type menu and setup response catch. */ 
 
void zedit_disp_comtype(struct descriptor_data *d) 
{ 
   get_char_cols(d->character); 
   send_to_char(d->character, "[H[J" 
	   "%sM%s) Load Mobile to room             %sO%s) Load Object to room\r\n" 
	   "%sE%s) Equip mobile with object        %sG%s) Give an object to a mobile\r\n" 
	   "%sP%s) Put object in another object    %sD%s) Open/Close/Lock a Door\r\n" 
	   "%sR%s) Remove an object from the room  %sF%s) Force previous mobile to...\r\n\r\n" 
	   "%sT%s) Assign a trigger                %sV%s) Set a global variable\r\n"
	   "What sort of command will this be? : ", 
	   grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn, nrm, 
	   grn, nrm, grn, nrm, grn, nrm  , grn, nrm  , grn, nrm  
      ); 
   OLC_MODE(d)=ZEDIT_COMMAND_TYPE; 
} 
 
 
/*-------------------------------------------------------------------*/ 
/*. Print the appropriate message for the command type for arg1 and set 
  up the input catch clause .*/ 
 
void zedit_disp_arg1(struct descriptor_data *d) 
{ 
   switch(OLC_CMD(d).command) 
      { 
       case 'M': 
	  send_to_char(d->character, "Input mob's vnum : "); 
	  OLC_MODE(d) = ZEDIT_ARG1; 
	  break; 
       case 'O': 
       case 'E': 
       case 'P': 
       case 'G': 
	  send_to_char(d->character, "Input object vnum : "); 
	  OLC_MODE(d) = ZEDIT_ARG1; 
	  break; 
       case 'F':
	  send_to_char(d->character, "Input Command String: ");
	  OLC_MODE(d) = ZEDIT_ARG1;
	  break;
       case 'D': 
       case 'R': 
	 /*. Arg1 for these is the room number, skip to arg2 .*/ 
	  OLC_CMD(d).arg1 = real_room(OLC_NUM(d)); 
	  OLC_CMD(d).oarg1= OLC_NUM(d);
	  zedit_disp_arg2(d); 
	  break; 
       case 'T':
       case 'V':
	  send_to_char(d->character, "Input trigger type (0:mob, 1:obj, 2:room) :");
	  OLC_MODE(d) = ZEDIT_ARG1;
	  break;
        default: 
	 /*. We should never get here .*/ 
	  cleanup_olc(d, CLEANUP_ALL); 
	  mudlogf(BRF,LVL_BUILDER,TRUE,"SYSERR: OLC: zedit_disp_arg1(): Help!");
	  return; 
      } 
} 
 
     
 
/*-------------------------------------------------------------------*/ 
/*. Print the appropriate message for the command type for arg2 and set 
  up the input catch clause .*/ 
 
void zedit_disp_arg2(struct descriptor_data *d) 
{ 
   int i = 0; 
   switch(OLC_CMD(d).command) 
      { 
       case 'O': 
	  OLC_CMD(d).arg2= 10;
	  OLC_CMD(d).arg3 = real_room(OLC_NUM(d)); 
	  OLC_CMD(d).oarg3 = OLC_NUM(d); 
	  zedit_disp_arg4(d); 
	  return;
	  break;

       case 'E': 
       case 'P': 
       case 'G': 
	  OLC_CMD(d).arg2= 10;
	  zedit_disp_arg3(d);
	  return;
	  break;
	  
       case 'M': 
	  send_to_char(d->character, "Input the maximum number that can exist on the mud : "); 
	  break; 
       case 'D': 
	  while(*dirs[i] != '\n') 
	     { 
	     send_to_char(d->character, "%d) Exit %s.\r\n", i, dirs[i]); 
	     i++; 
	     } 
	  send_to_char(d->character, "Enter exit number for door : "); 
	  break; 
       case 'R': 
	  send_to_char(d->character, "Input object's vnum : "); 
	  break; 
       case 'T':
	  send_to_char(d->character, "Enter the trigger VNum : ");
	  break;
       case 'V':
	  send_to_char(d->character, "Global's context (0 for none) : ");
	  break;
       default: 
	 /*. We should never get here .*/ 
	  cleanup_olc(d, CLEANUP_ALL); 
	  mudlogf(BRF, LVL_BUILDER, TRUE,
		 "SYSERR: OLC: zedit_disp_arg2(): Help!"); 
	  return; 
      } 
   OLC_MODE(d) = ZEDIT_ARG2; 
} 
 
 
/*-------------------------------------------------------------------*/ 
/*. Print the appropriate message for the command type for arg3 and set 
  up the input catch clause .*/ 
 
void zedit_disp_arg3(struct descriptor_data *d) 
{ 
   int i = 0; 

   switch(OLC_CMD(d).command) 
      {  
       case 'E': 
	  while(*equipment_types[i] !=  '\n') 
	     { 
	     send_to_char(d->character, "%2d) %26.26s %2d) %26.26s\r\n", i,  
		     equipment_types[i], i+1, (*equipment_types[i+1] != '\n') ? 
		     equipment_types[i+1] : ""); 
	     if(*equipment_types[i+1] != '\n') 
		i+=2; 
	     else 
		break; 
	     } 
	  send_to_char(d->character, "Input location to equip : "); 
	  break; 
       case 'P': 
	  send_to_char(d->character, "Input the vnum of the container : "); 
	  break; 
       case 'D': 
	  send_to_char(d->character, "0)  Door open\r\n" 
		       "1)  Door closed\r\n" 
		       "2)  Door locked\r\n"  
		       "Enter state of the door : "); 
	  break; 
       case 'G': 
	  send_to_char(d->character, "Give the percentage chance that this event should happen: ");
	  break;
       case 'V':
	  send_to_char(d->character, "Enter the global name: ");
	  OLC_MODE(d) = ZEDIT_SARG1;
	  return;
       case 'M': 
       case 'O': 
       case 'R': 
       case 'T': 
       default: 
	 /*. We should never get here .*/ 
	  cleanup_olc(d, CLEANUP_ALL); 
	  mudlogf(BRF, LVL_BUILDER, TRUE,
		 "SYSERR: OLC: zedit_disp_arg3(): Help!"); 
	  return; 
      } 
   OLC_MODE(d) = ZEDIT_ARG3; 
} 


/*-------------------------------------------------------------------*/
/*. Print the appropriate message for the command type for arg4 and set
  up the input catch clause .*/

void zedit_disp_arg4(struct descriptor_data *d)
{
   switch(OLC_CMD(d).command)
      {
       case 'E':
       case 'O':
       case 'P':
	  send_to_char(d->character, "Give the percentage chance that this event should happen: ");
	  break;
	  
       case 'M':
       case 'G':
       case 'D':
       case 'R':
       default:
	 /*. We should never get here .*/
	  cleanup_olc(d, CLEANUP_ALL);
	  mudlogf(BRF,LVL_IMMORT,TRUE,
		 "SYSERR: OLC: zedit_disp_arg3(): Help!");
	  return;
      }
   OLC_MODE(d) = ZEDIT_ARG4;
}

 
void zedit_disp_zone_flags(struct descriptor_data *d)
{
   int i, columns=0;
   char *buf2=get_buffer(SMALL_BUFSIZE);

   get_char_cols(d->character);
#if defined(CLEAR_SCREEN)
   send_to_char(d->character, "[H[J"); 
#endif
   for (i = 0; i < NUM_ZONE_FLAGS; i++) 
      {  
      send_to_char(d->character, "%s%2d%s) %-20.20s  ", grn, i+1, nrm, 
		   zone_bits[i]); 
      if(!(++columns % 2)) 
         send_to_char(d->character, "\r\n"); 
      } 
   sprintbit(OLC_ZONE(d)->bitvector, zone_bits, buf2); 
   send_to_char(d->character, "\r\n" 
           "Current flags   : %s%s%s\r\n" 
           "Enter zone flags (0 to quit) : ", 
           cyn, buf2, nrm ); 
   release_buffer(buf2);
}   


void zedit_disp_zone_status(struct descriptor_data *d)
{
   int i;

   get_char_cols(d->character);
#if defined(CLEAR_SCREEN)
   send_to_char(d->character, "[H[J"); 
#endif
   for (i = 0; i < NUM_ZONE_STATUS; i++) 
      {  
      send_to_char(d->character, "%s%2d%s) %-20.20s\r\n", grn, i+1, nrm, 
		   zone_status[i] ); 
      } 
   send_to_char(d->character, "\r\n" 
           "Current status   : %s%s%s\r\n" 
           "Enter zone status (0 to quit) : ", 
           cyn, zone_status[(int)OLC_ZONE(d)->status], nrm ); 
}   

void zedit_disp_zone_continent(struct descriptor_data *d)
{
   int i;

   get_char_cols(d->character);
#if defined(CLEAR_SCREEN)
   send_to_char(d->character, "[H[J"); 
#endif
   for (i = 0; i < NUM_ZONE_CONTINENT; i++) 
      {  
      send_to_char(d->character, "%s%2d%s) %s%-20.20s%s\r\n", grn,i+1,nrm,
	      cyn,zone_continent[i][0],nrm); 
      } 
   send_to_char(d->character, "\r\n" 
           "Current continent: %s%s%s\r\n" 
           "Enter zone continent (0 to quit) : ", 
           cyn,
	   zone_continent[(int)OLC_ZONE(d)->continent][0], nrm ); 
}   

void zedit_disp_zone_source(struct descriptor_data *d)
{
   int i;

   get_char_cols(d->character);
#if defined(CLEAR_SCREEN)
   send_to_char(d->character, "[H[J"); 
#endif
   for (i = 0; i < NUM_ZONE_SOURCE; i++) 
      {  
      send_to_char(d->character, "%s%2d%s) %-20.20s\r\n", grn, i+1, nrm, 
		   zone_source[i] ); 
      } 
   send_to_char(d->character, "\r\n" 
           "Current source   : %s%s%s\r\n" 
           "Enter zone source (0 to quit) : ", 
           cyn, zone_source[(int)OLC_ZONE(d)->source], nrm ); 
}   
     
     
/************************************************************************** 
  The GARGANTAUN event handler 
 **************************************************************************/ 
 
void zedit_parse(struct descriptor_data * d, char *arg) 
{ 
   int pos, i = 0; 
   char *buf;

   switch (OLC_MODE(d))  
      { 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_CONFIRM_SAVESTRING: 
	  switch (*arg) 
	     { 
	      case 'y': 
	      case 'Y': 
		/*. Save the zone in memory .*/ 
		 send_to_char(d->character, "Saving zone info in memory.\r\n");
		 OLC_ZONE(d)->dateLastMod=time(0);
		 free(OLC_ZONE(d)->nameLastMod);

		 buf=get_buffer(MAX_STRING_LENGTH);
		 sprintf(buf,"%s - zone",GET_NAME(d->character));
		 OLC_ZONE(d)->nameLastMod = str_dup(buf);
		 release_buffer(buf);

		 zedit_save_internally(d); 
		 mudlogf(CMP, MAX(LVL_BUILDER,GET_INVIS_LEV(d->character)), 
			 TRUE,"OLC: %s has edited zone info for room %d", 
			 GET_NAME(d->character), OLC_NUM(d)); 
	      case 'n': 
	      case 'N': 
		 cleanup_olc(d, CLEANUP_ALL); 
		 break; 
	      default: 
		 send_to_char(d->character, "Invalid choice!\r\n"); 
		 send_to_char(d->character, "Do you wish to save the zone info? : "); 
		 break; 
	     } 
	  break; /* end of ZEDIT_CONFIRM_SAVESTRING */ 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_MAIN_MENU: 
	  switch (*arg)  
	     { 
	      case 'q': 
	      case 'Q': 
		 if(OLC_ZONE(d)->age || OLC_ZONE(d)->number) 
		    { 
		    send_to_char(d->character, "Do you wish to save the changes to the zone info? (y/n) : "); 
		    OLC_MODE(d) = ZEDIT_CONFIRM_SAVESTRING; 
		    } 
		 else 
		    { 
		    send_to_char(d->character, "No changes made.\r\n"); 
		    cleanup_olc(d, CLEANUP_ALL); 
		    } 
		 break; 
	      case 'n': 
	      case 'N': 
		/*. New entry .*/ 
		 send_to_char(d->character, "What number in the list should the new command be? : "); 
		 OLC_MODE(d) = ZEDIT_NEW_ENTRY; 
		 break; 
	      case 'e': 
	      case 'E': 
		/*. Change an entry .*/ 
		 send_to_char(d->character, "Which command do you wish to change? : "); 
		 OLC_MODE(d) = ZEDIT_CHANGE_ENTRY; 
		 break; 
	      case 'd': 
	      case 'D': 
		/*. Delete an entry .*/ 
		 send_to_char(d->character, "Which command do you wish to delete? : "); 
		 OLC_MODE(d) = ZEDIT_DELETE_ENTRY; 
		 break; 
	      case 'z': 
	      case 'Z': 
		/*. Edit zone name .*/ 
		 send_to_char(d->character, "Enter new zone name : "); 
		 OLC_MODE(d) = ZEDIT_ZONE_NAME; 
		 break; 
	      case 'a': 
	      case 'A': 
		 if(GET_LEVEL(d->character) < LVL_IMPL) 
		    zedit_disp_menu(d); 
		 else 
		    { 
		    send_to_char(d->character, "Enter new zone author : "); 
		    OLC_MODE(d) = ZEDIT_ZONE_AUTHOR; 
		    }
		 break; 
	      case 'b': 
	      case 'B': 
		 if(GET_LEVEL(d->character) < LVL_IMPL) 
		    zedit_disp_menu(d); 
		 else 
		    { 
		    send_to_char(d->character, "Enter new zone editor : "); 
		    OLC_MODE(d) = ZEDIT_ZONE_EDITOR; 
		    }
		 break; 
         case 'j': 
         case 'J': 
       if(GET_LEVEL(d->character) < LVL_ADMIN) 
          zedit_disp_menu(d); 
       else 
          { 
          send_to_char(d->character, "Enter new world proofer : "); 
          OLC_MODE(d) = ZEDIT_WORLD_PROOF; 
          }
       break; 
         case 'k': 
         case 'K': 
       if(GET_LEVEL(d->character) < LVL_ADMIN) 
          zedit_disp_menu(d); 
       else 
          { 
          send_to_char(d->character, "Enter new trigger proofer : "); 
          OLC_MODE(d) = ZEDIT_TRIG_PROOF; 
          }
       break; 
         case 'm': 
         case 'M': 
       if(GET_LEVEL(d->character) < LVL_ADMIN) 
          zedit_disp_menu(d); 
       else 
          { 
          send_to_char(d->character, "Enter new object balancer : "); 
          OLC_MODE(d) = ZEDIT_OBJ_PROOF; 
          }
       break; 
	      case 'v': 
	      case 'V': 
		 send_to_char(d->character, "Enter new zone level string : "); 
		 OLC_MODE(d) = ZEDIT_ZONE_LEVELS; 
		 break; 
	      case 't': 
	      case 'T': 
		/*. Edit zone top .*/ 
		 if(GET_LEVEL(d->character) < LVL_IMPL) 
		    zedit_disp_menu(d); 
		 else 
		    { 
		    send_to_char(d->character, "Enter new top of zone : "); 
		    OLC_MODE(d) = ZEDIT_ZONE_TOP; 
		    } 
		 break; 
	      case 'c': 
	      case 'C': 
		 zedit_disp_zone_status(d);
		 OLC_MODE(d) = ZEDIT_ZONE_STATUS; 
		 break; 
	      case 'g': 
	      case 'G': 
		 if(GET_LEVEL(d->character) < LVL_ADMIN) 
		    zedit_disp_menu(d); 
		 else 
		    { 
		    zedit_disp_zone_source(d);
		    OLC_MODE(d) = ZEDIT_ZONE_SOURCE; 
		    } 
		 break; 
	      case 'h': 
	      case 'H': 
		 zedit_disp_zone_continent(d);
		 OLC_MODE(d) = ZEDIT_ZONE_CONTINENT; 
		 break; 
	      case 'l': 
	      case 'L': 
		/*. Edit zone lifespan .*/ 
		 send_to_char(d->character, "Enter new zone lifespan : "); 
		 OLC_MODE(d) = ZEDIT_ZONE_LIFE; 
		 break; 
	      case 'r': 
	      case 'R': 
		/*. Edit zone reset mode .*/ 
		 send_to_char(d->character, "\r\n" 
			      "0) Never reset\r\n" 
			      "1) Reset only when no players in zone (can idle)\r\n" 
			      "2) Normal reset (can idle)\r\n" 
			      "3) Normal reset (no idle)\r\n"
			      "Enter new zone reset type : "); 
		 OLC_MODE(d) = ZEDIT_ZONE_RESET; 
		 break; 
	      case 'f':
	      case 'F':
		 zedit_disp_zone_flags(d);
		 OLC_MODE(d) = ZEDIT_ZONE_FLAGS; 
		 break;
	      case 'i': 
	      case 'I': 
		 send_to_char(d->character, "Enter new comment : "); 
		 OLC_MODE(d) = ZEDIT_ZONE_COMMENT; 
		 break; 
	      default: 
		 zedit_disp_menu(d); 
		 break; 
	     } 
	  break; /*. End ZEDIT_MAIN_MENU .*/ 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_NEW_ENTRY: 
	 /*. Get the line number and insert the new line .*/ 
	  pos = atoi(arg); 
	  if (isdigit((int)*arg) && new_command(d, pos)) 
	     {  
	     if (start_change_command(d, pos)) 
		{ 
		zedit_disp_comtype(d); 
		OLC_ZONE(d)->age = 1; 
		} 
	     } 
	  else 
	     zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_DELETE_ENTRY: 
	 /*. Get the line number and delete the line .*/ 
	  pos = atoi(arg); 
	  if(isdigit((int)*arg)) 
	     { 
	     delete_command(d, pos); 
	     OLC_ZONE(d)->age = 1; 
	     } 
	  zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_CHANGE_ENTRY: 
	 /*. Parse the input for which line to edit, and goto next quiz .*/ 
	  pos = atoi(arg); 
	  if(isdigit((int)*arg) && start_change_command(d, pos)) 
	     { 
	     zedit_disp_comtype(d); 
	     OLC_ZONE(d)->age = 1; 
	     } 
	  else 
	     zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_COMMAND_TYPE: 
	 /*. Parse the input for which type of command this is,  
	   and goto next quiz .*/ 
	  OLC_CMD(d).command = toupper(*arg); 
	  if (!OLC_CMD(d).command || (strchr("MOPEDGRFTV", OLC_CMD(d).command) == NULL)) 
	     { 
	     send_to_char(d->character, "Invalid choice, try again : "); 
	     } 
	  else 
	     { 
	     if (OLC_VAL(d)) 
		{ 
	       /*. If there was a previous command .*/ 
		if (OLC_CMD(d).command == 'T' || OLC_CMD(d).command == 'V') 
		   {
		   OLC_CMD(d).if_flag = 1;
		   zedit_disp_arg1(d);
		   }
		else 
		   {
		   send_to_char(d->character, "Is this command dependent on the success of the previous one? (y/n)\r\n");
		   OLC_MODE(d) = ZEDIT_IF_FLAG;
		   }
		} 
	     else 
		{ 
/*. 'if-flag' not appropriate .*/ 
		OLC_CMD(d).if_flag = 0; 
		zedit_disp_arg1(d); 
		} 
	     } 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_IF_FLAG: 
	 /*. Parse the input for the if flag, and goto next quiz .*/ 
	  switch(*arg) 
	     { 
	      case 'y': 
	      case 'Y': 
		 OLC_CMD(d).if_flag = 1; 
		 break; 
	      case 'n': 
	      case 'N': 
		 OLC_CMD(d).if_flag = 0; 
		 break; 
	      default: 
		 send_to_char(d->character, "Try again : "); 
		 return; 
	     } 
	  zedit_disp_arg1(d); 
	  break; 
 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ARG1: 
	 /*. Parse the input for arg1, and goto next quiz .*/ 
	  if  (!isdigit((int)*arg)&&(OLC_CMD(d).command !='F')) 
	     { 
	     send_to_char(d->character, "Must be a numeric value, try again : "); 
	     return; 
	     } 
	  switch(OLC_CMD(d).command) 
	     { 
	      case 'M': 
		 pos = real_mobile(atoi(arg)); 
		 if (pos >= 0) 
		    { 
		    OLC_CMD(d).arg1 = pos; 
		    OLC_CMD(d).oarg1 = atoi(arg); 
		    zedit_disp_arg2(d); 
		    } 
		 else 
		    send_to_char(d->character, "That mobile does not exist, try again : "); 
		 break; 
	      case 'P': 
	      case 'E': 
	      case 'O': 
	      case 'G': 
		 pos = real_object(atoi(arg)); 
		 if (pos >= 0) 
		    { 
		    OLC_CMD(d).arg1 = pos; 
		    OLC_CMD(d).oarg1 = atoi(arg); 
		    zedit_disp_arg2(d); 
		    } 
		 else 
		    send_to_char(d->character, "That object does not exist, try again : "); 
		 break; 
	      case 'F':
		 OLC_CMD(d).sarg=str_dup(arg);
		 zedit_disp_menu(d); 
		 break;
	      case 'T':
	      case 'V':
		 if (atoi(arg)<MOB_TRIGGER || atoi(arg)>WLD_TRIGGER)
		    send_to_char(d->character, "Invalid input.");
		 else 
		    {
		    OLC_CMD(d).arg1 = atoi(arg);
		    zedit_disp_arg2(d);
		    }
		 break;
	      case 'D': 
	      case 'R': 
	      default: 
		/*. We should never get here .*/ 
		 cleanup_olc(d, CLEANUP_ALL); 
		 mudlogf(BRF, LVL_BUILDER, TRUE,
			"SYSERR: OLC: zedit_parse(): case ARG1: Ack!"); 
		 break; 
	     } 
	  break; 
 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ARG2: 
	 /*. Parse the input for arg2, and goto next quiz .*/ 
	  if  (!isdigit((int)*arg)) 
	     { 
	     send_to_char(d->character, "Must be a numeric value, try again : "); 
	     return; 
	     } 
	  switch(OLC_CMD(d).command) 
	     { 
	      case 'M': 
		 OLC_CMD(d).arg2 = atoi(arg); 
		 OLC_CMD(d).arg3 = real_room(OLC_NUM(d)); 
		 OLC_CMD(d).oarg3 = OLC_NUM(d); 
		 zedit_disp_menu(d); 
		 break; 
	      case 'O': 
		 OLC_CMD(d).arg2 = atoi(arg); 
		 OLC_CMD(d).arg3 = real_room(OLC_NUM(d)); 
		 OLC_CMD(d).oarg3 = OLC_NUM(d); 
		 zedit_disp_arg4(d); 
		 break; 
	      case 'G': 
		 OLC_CMD(d).arg2 = atoi(arg); 
		 zedit_disp_arg3(d); 
		 break;  
	      case 'P': 
	      case 'E': 
		 OLC_CMD(d).arg2 = atoi(arg); 
		 zedit_disp_arg3(d); 
		 break; 
	      case 'V':
		 OLC_CMD(d).arg2 = atoi(arg);
		 if (OLC_CMD(d).arg1 == WLD_TRIGGER)
		    OLC_CMD(d).arg3 = OLC_NUM(d);
		 send_to_char(d->character, "Enter the global name : ");
		 OLC_MODE(d) = ZEDIT_SARG1;
		 break;
	      case 'D': 
		 pos = atoi(arg); 
		/*. Count dirs .*/ 
		 while(*dirs[i] != '\n') 
		    i++; 
		 if ((pos < 0) || (pos > i)) 
		    send_to_char(d->character, "Try again : "); 
		 else 
		    { 
		    OLC_CMD(d).arg2 = pos; 
		    zedit_disp_arg3(d); 
		    } 
		 break; 
	      case 'T':
		 if (real_trigger(atoi(arg)) >= 0)
		    {
		    OLC_CMD(d).arg2 = atoi(arg);
		    zedit_disp_menu(d);
		    } 
		 else
		    send_to_char(d->character, "That trigger does not exist, try again : ");
		 break;
	      case 'R': 
		 pos = real_object(atoi(arg)); 
		 if (pos >= 0) 
		    { 
		    OLC_CMD(d).arg2 = pos; 
		    OLC_CMD(d).oarg2 = atoi(arg); 
		    zedit_disp_menu(d); 
		    } 
		 else 
		    send_to_char(d->character, "That object does not exist, try again : "); 
		 break; 
	      default: 
		/*. We should never get here .*/ 
		 cleanup_olc(d, CLEANUP_ALL); 
		 mudlogf(BRF, LVL_BUILDER, TRUE,
			"SYSERR: OLC: zedit_parse(): case ARG2: Ack!"); 
		 break; 
	     } 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ARG3: 
	 /*. Parse the input for arg3, and go back to main menu.*/ 
	  if  (!isdigit((int)*arg)&&(OLC_CMD(d).command!='V')) 
	     { 
	     send_to_char(d->character, "Must be a numeric value, try again : "); 
	     return; 
	     } 
	  switch(OLC_CMD(d).command) 
	     { 
	      case 'E': 
		 pos = atoi(arg); 
		/*. Count number of wear positions (could use NUM_WEARS, 
		  this is more reliable) .*/ 
		 while(*equipment_types[i] != '\n') 
		    i++; 
		 if ((pos < 0) || (pos > i)) 
		    send_to_char(d->character, "Try again : "); 
		 else 
		    { 
		    OLC_CMD(d).arg3 = pos; 
		    zedit_disp_arg4(d); 
		    } 
		 break; 
	      case 'P': 
		 pos = real_object(atoi(arg)); 
		 if (pos >= 0) 
		    { 
		    OLC_CMD(d).arg3 = pos; 
		    OLC_CMD(d).oarg3 = atoi(arg); 
		    zedit_disp_arg4(d); 
		    } 
		 else 
		    send_to_char(d->character, "That object does not exist, try again : "); 
		 break; 
	      case 'D': 
		 pos = atoi(arg); 
		 if ((pos < 0) || (pos > 2)) 
		    send_to_char(d->character, "Try again : "); 
		 else 
		    { 
		    OLC_CMD(d).arg3 = pos; 
		    zedit_disp_menu(d); 
		    } 
		 break; 
	      case 'G': 
		 pos = atoi(arg);
		 if (pos == 100) 
		    {
		    OLC_CMD(d).arg3 = 0;
		    zedit_disp_menu(d);
		    }
		 else if (pos > 0 && pos < 100) 
		    {
		    OLC_CMD(d).arg3 = 101 - pos;
		    zedit_disp_menu(d);
		    }
		 else
		    send_to_char(d->character, "Give a number between 1 and 100. Try again: ");
		 break;
	      case 'V': 
	      case 'T': 
	      case 'M': 
	      case 'O': 
	      case 'R': 
	      default: 
		/*. We should never get here .*/ 
		 cleanup_olc(d, CLEANUP_ALL); 
		 mudlogf(BRF, LVL_BUILDER, TRUE,
			"SYSERR: OLC: zedit_parse(): case ARG3: Ack!"); 
		 break; 
	     } 
	  break; 

 
 /*-------------------------------------------------------------------*/
       case ZEDIT_ARG4:
	 /*. Parse the input for arg4, and go back to main menu. */
	  if (!isdigit((int)*arg)) 
	     {
	     send_to_char(d->character, "Must be a numeric value, try again : ");
	     return;
	     }
	  switch(OLC_CMD(d).command)
	     {
	      case 'E':
	      case 'O':
	      case 'P':
		 pos = atoi(arg);
		 if (pos == 100)
		    {
		    OLC_CMD(d).arg4 = 0;
		    zedit_disp_menu(d);
		    }
		 else if (pos > 0 && pos < 100) 
		    {
		    OLC_CMD(d).arg4 = 101 - pos;
		    zedit_disp_menu(d);
		    }
		 else
		    send_to_char(d->character, "Give a number between 1 and 100. Try again: ");
		 break;
	      case 'M':
	      case 'R':
	      case 'D':
	      default:
		/*. We should never get here .*/
		 cleanup_olc(d, CLEANUP_ALL);
		 mudlogf(BRF, LVL_IMMORT, TRUE,
			"SYSERR: OLC: zedit_parse(): case ARG3: Ack!");
		 break;
	     }
	  break; 
	  

/*-------------------------------------------------------------------*/
       case ZEDIT_SARG1:
	  if (strlen(arg)) 
	     {
	     OLC_CMD(d).sarg = str_dup(arg);
	     OLC_MODE(d) = ZEDIT_SARG2;
	     send_to_char(d->character, "Enter the global value : ");
	     } 
	  else
	     send_to_char(d->character, "Must have some name to assign : ");
	  break;
	  
/*-------------------------------------------------------------------*/
       case ZEDIT_SARG2:
	  if (strlen(arg)) 
	     {
	     OLC_CMD(d).sarg2 = str_dup(arg);
	     zedit_disp_menu(d);
	     } 
	  else
	     send_to_char(d->character, "Must have some value to set it to :");
	  break;
   
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_NAME: 
	 /*. Add new name and return to main menu .*/ 
	  free(OLC_ZONE(d)->name); 
	  if((*arg=='*')||(*arg=='~'))
	     OLC_ZONE(d)->name = str_dup("Must start with a letter."); 
	  else
	     OLC_ZONE(d)->name = str_dup(arg); 
	  OLC_ZONE(d)->number = 1; 
	  zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_AUTHOR: 
	 /*. Add new name and return to main menu .*/ 
	  free(OLC_ZONE(d)->author); 
	  if(!isalpha((int)*arg))
	     OLC_ZONE(d)->author = str_dup("Must start with a letter."); 
	  else
	     OLC_ZONE(d)->author = str_dup(arg); 
	  OLC_ZONE(d)->number = 1; 
	  zedit_disp_menu(d); 
	  break; 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_COMMENT:
	 /*. Add new comment and return to main menu .*/ 
	  free(OLC_ZONE(d)->comment); 
	  if(!isalpha((int)*arg))
	     OLC_ZONE(d)->comment = str_dup("Must start with a letter."); 
	  else
	     OLC_ZONE(d)->comment = str_dup(arg); 
	  OLC_ZONE(d)->number = 1; 
	  zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_EDITOR: 
	 /*. Add new name and return to main menu .*/ 
	  free(OLC_ZONE(d)->editor); 
	  if(!isalpha((int)*arg))
	     OLC_ZONE(d)->editor = str_dup("Must start with a letter."); 
	  else
	     OLC_ZONE(d)->editor = str_dup(arg); 
	  OLC_ZONE(d)->number = 1; 
	  zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_LEVELS: 
	 /*. Add new name and return to main menu .*/ 
	  free(OLC_ZONE(d)->levels); 
	  if(!isalnum((int)*arg))
	     OLC_ZONE(d)->levels = str_dup("Must start with a letter or number."); 
	  else
	     OLC_ZONE(d)->levels = str_dup(arg); 
	  OLC_ZONE(d)->number = 1; 
	  zedit_disp_menu(d); 
	  break; 
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_WORLD_PROOF: 
    /*. Add new name and return to main menu .*/ 
   free(OLC_ZONE(d)->worldProof); 
     if(!isalpha((int)*arg))
      OLC_ZONE(d)->worldProof = str_dup("Must start with a letter."); 
     else
       OLC_ZONE(d)->worldProof = str_dup(arg); 
     OLC_ZONE(d)->number = 1; 
    zedit_disp_menu(d); 
   break; 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_TRIG_PROOF: 
    /*. Add new name and return to main menu .*/ 
   free(OLC_ZONE(d)->trigProof); 
     if(!isalpha((int)*arg))
      OLC_ZONE(d)->trigProof = str_dup("Must start with a letter."); 
     else
       OLC_ZONE(d)->trigProof = str_dup(arg); 
     OLC_ZONE(d)->number = 1; 
    zedit_disp_menu(d); 
   break; 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_OBJ_PROOF: 
    /*. Add new name and return to main menu .*/ 
   free(OLC_ZONE(d)->objBalanced); 
     if(!isalpha((int)*arg))
      OLC_ZONE(d)->objBalanced = str_dup("Must start with a letter."); 
     else
       OLC_ZONE(d)->objBalanced = str_dup(arg); 
     OLC_ZONE(d)->number = 1; 
    zedit_disp_menu(d); 
   break; 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_RESET: 
	 /*. Parse and add new reset_mode and return to main menu .*/ 
	  pos = atoi(arg); 
	  if (!isdigit((int)*arg) || (pos <  0) || (pos > 3)) 
	     send_to_char(d->character, "Try again (0-3) : "); 
	  else 
	     { 
	     OLC_ZONE(d)->reset_mode = pos; 
	     OLC_ZONE(d)->number = 1; 
	     zedit_disp_menu(d); 
	     } 
	  break;  
 
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_FLAGS: 
          pos = atoi(arg); 
          if (pos==0) 
	     {
	     zedit_disp_menu(d);
             break; 
	     }
          if (!((pos < 0) || (pos > NUM_ZONE_FLAGS))) 
             { 
             pos = 1 << (pos - 1); 
             if (IS_SET(OLC_ZONE(d)->bitvector, pos)) 
                REMOVE_BIT(OLC_ZONE(d)->bitvector, pos); 
             else 
                SET_BIT(OLC_ZONE(d)->bitvector, pos); 
	     OLC_ZONE(d)->number = 1; 
             } 
          zedit_disp_zone_flags(d); 
          return; 

/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_LIFE: 
	 /*. Parse and add new lifespan and return to main menu .*/ 
	  pos = atoi(arg); 
	  if (!isdigit((int)*arg) || (pos <  0) || (pos > 240)) 
	     send_to_char(d->character, "Try again (0-240) : "); 
	  else 
	     { 
	     OLC_ZONE(d)->lifespan = pos; 
	     OLC_ZONE(d)->number = 1; 
	     zedit_disp_menu(d); 
	     } 
	  break;  

/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_SOURCE: 
          pos = atoi(arg); 
          if (pos==0) 
	     {
	     zedit_disp_menu(d);
             break; 
	     }
          if (!((pos < 0) || (pos > NUM_ZONE_SOURCE))) 
             { 
	     OLC_ZONE(d)->source = pos-1; 
	     OLC_ZONE(d)->number = 1; 
	     }
	  zedit_disp_menu(d); 
	  break;  
	  
/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_STATUS: 
          pos = atoi(arg); 
          if (pos==0) 
	     {
	     zedit_disp_menu(d);
             break; 
	     }
	  if((GET_LEVEL(d->character)<LVL_ADMIN)&&(pos!=2)&&
	     (OLC_ZONE(d)->status!=0))
	     {
	     zedit_disp_menu(d);
             break; 
	     }
          if (!((pos < 0) || (pos > NUM_ZONE_STATUS))) 
             {
	     OLC_ZONE(d)->status = pos-1; 
	     OLC_ZONE(d)->number = 1; 
	     }
	  if(OLC_ZONE(d)->status == 4)
	     {
	     OLC_ZONE(d)->dateImped=time(0);
	     }
	  else
	     {
	     OLC_ZONE(d)->dateImped=0;
	     }
	  zedit_disp_menu(d); 
	  break;  

/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_CONTINENT: 
          pos = atoi(arg); 
          if (pos==0) 
	     {
	     zedit_disp_menu(d);
             break; 
	     }
          if (!((pos < 0) || (pos > NUM_ZONE_CONTINENT))) 
             {
	     OLC_ZONE(d)->continent = pos-1; 
	     OLC_ZONE(d)->number = 1; 
	     }
	  zedit_disp_menu(d); 
	  break;  

/*-------------------------------------------------------------------*/ 
       case ZEDIT_ZONE_TOP: 
	 /*. Parse and add new top room in zone and return to main menu .*/ 
	  if (OLC_ZNUM(d) == top_of_zone_table) 
	     OLC_ZONE(d)->top = MAX(OLC_ZNUM(d) * 100, MIN(32000, atoi(arg))); 
	  else 
	     OLC_ZONE(d)->top = MAX(OLC_ZNUM(d) * 100, MIN(zone_table[OLC_ZNUM(d) +1].number * 100, atoi(arg))); 
	  zedit_disp_menu(d); 
	  break;  
 
/*-------------------------------------------------------------------*/ 
       default: 
	 /*. We should never get here .*/ 
	  cleanup_olc(d, CLEANUP_ALL); 
	  mudlogf(BRF,LVL_BUILDER,TRUE,
		 "SYSERR: OLC: zedit_parse(): Reached default case!"); 
	  send_to_char(d->character, "Oops...\r\n");
	  break; 
      } 

} 
/*. End of parse_zedit() .*/ 
 








/* Test comment */
/* Test comment */
/* Test comment */
/* Test comment */


