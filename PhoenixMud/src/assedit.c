 ...
 * Function prototypes.
 *-------------------------------------------------------------------*/
void assedit_setup(struct descriptor_data *d, int anumber);
...
ACMD (do_assedit)
   ...
   for (d = descriptor_list; d; d = d->next)
      ...
      if (d->connected == CON_ASSEDIT)
         {
         send_to_char(ch, "Assemblies are already being edited by someone.\r\n");
         return;
         }
      }
...
void assedit_setup(struct descriptor_data *d, int anumber)
   ...
   if( (pOldAssembly = assemblyGetAssemblyPtr( anumber )) == NULL )
      {
...
void assedit_disp_menu(struct descriptor_data *d)
   ...
   sprinttype( OLC_ASSEDIT(d)->uchAssemblyType, AssemblyTypes, buf );
