...
{ 
   FILE         *ptFHndl; 
   char         *pcFileName=get_buffer(128); 
   struct alias_data *pstAliasRec; 
   char         *pcRepStart; 
...
{    
  ...
  /* create initial alias record */ 
   CREATE(pstAliasRec, struct alias_data, 1); 
  ...
  /* while not end of alias file */ 
   while (!feof(ptFHndl)) 
	...
	/* create new alias record and link into chain */ 
	 CREATE(pstAliasRec->next, struct alias_data, 1); 
	 pstAliasRec = pstAliasRec->next; 
