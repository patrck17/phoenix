...
void sedit_add_to_int_list(long **list, int new) 
{ 
...
void sedit_remove_from_int_list(long **list, int num) 
{ 
   long i, num_items, *nlist; 
...
{ 
   ...
   sprintf(buf,"%s - shop",GET_NAME(d->character));
...
{  
   ...
   sprintf(fname, "%s/%ld.new", SHP_PREFIX, zone); 
 
   if(!(shop_file = fopen(fname, "w"))) 
      {  
      mudlogf(BRF, LVL_BUILDER, TRUE,"SYSERR: OLC: Cannot open shop file!"); 
   ...
   if (fprintf(shop_file, "CircleMUD v3.0 Shop File~\n") < 0) 
      ...
      mudlogf(BRF, LVL_BUILDER, TRUE,
	     "SYSERR: OLC: Cannot write to shop file!"); 
      release_buffer(fname);
      return; 
      } 
  ...
  /*. Search database for shops in this zone .*/ 
   ...
   fclose(shop_file); 
   buf2=get_buffer(265);
   sprintf(buf2, "%s/%ld.shp", SHP_PREFIX, zone);
  /*
   * We're fubar'd if we crash between the two lines below.
   */
   remove(buf2);
   rename(fname, buf2);
   zedit_create_index(zone,"shp");
   release_buffer(buf2);
   release_buffer(fname);
...
{ 
   ...
   send_to_char(d->character, "[H[J##     VNUM     Product\r\n"); 
...
{ 
   ...
   send_to_char(d->character, "[H[J##     VNUM     Room\r\n\r\n"); 
   for(i=0;  S_ROOM(shop, i) != -1; i++) 
      { 
      send_to_char(d->character, 
	      "%2d - [%s%5ld%s] - %s%s%s\r\n",   
...
{ 
   ...
   send_to_char(d->character, "[H[J##              Type   Namelist\r\n\r\n"); 
...
{ 
   ...
   sprintbit(S_BITVECTOR(OLC_SHOP(d)), shop_bits, buf2); 
   send_to_char(d->character, 
...
{ 
   ...
   sprintbit(S_NOTRADE(OLC_SHOP(d)), trade_letters, buf2); 
   send_to_char(d->character, 
...
{ 
   ...
   send_to_char(d->character, "%sEnter choice : ", nrm); 
...
/*-------------------------------------------------------------------*/ 
	  ...
	  switch (*arg) 
	     { 
	      case 'y': 
	      case 'Y': 
		 send_to_char(d->character, "Saving shop to memory.\r\n"); 
		 sedit_save_internally(d); 
		 mudlogf(CMP, LVL_BUILDER, TRUE, "OLC: %s has edited shop %d", 
		 ...
		 send_to_char(d->character,"Do you wish to save the shop? : ");
		 return; 
	     } 
	  break; 
...
/*-------------------------------------------------------------------*/ 
	  ...
	  switch (*arg)  
		 ...
		 if (OLC_VAL(d)) /*. Anything been changed? .*/ 
		    { 
		    send_to_char(d->character, "Do you wish to save the changes to the shop? (y/n) : "); 
		 ...
		 send_to_char(d->character, "Enter virtual number of shop keeper : "); 
		 return; 
	      case '1': 
...
/*-------------------------------------------------------------------*/ 
	 ...
	 /*. Fiddle with special procs .*/ 
	  ...
	  if(S_BUYPROFIT(OLC_SHOP(d))<0.1)
	     S_BUYPROFIT(OLC_SHOP(d))=0.1;
	  if(S_BUYPROFIT(OLC_SHOP(d))<=S_SELLPROFIT(OLC_SHOP(d)))
	     S_BUYPROFIT(OLC_SHOP(d))=S_SELLPROFIT(OLC_SHOP(d))+0.1;
	  break; 
       case SEDIT_SELL_PROFIT: 
	  sscanf(arg, "%f", &S_SELLPROFIT(OLC_SHOP(d))); 
	  if(S_SELLPROFIT(OLC_SHOP(d))<0.1)
	     S_SELLPROFIT(OLC_SHOP(d))=0.1;
	  if(S_BUYPROFIT(OLC_SHOP(d))<=S_SELLPROFIT(OLC_SHOP(d)))
	     S_SELLPROFIT(OLC_SHOP(d))=S_BUYPROFIT(OLC_SHOP(d))-0.1;
	  break; 
       case SEDIT_TYPE_MENU: 
	  OLC_VAL(d) = MAX(0, MIN(NUM_ITEM_TYPES - 1, atoi(arg))); 
	  send_to_char(d->character, "Enter namelist (return for none) :-\r\n| "); 
