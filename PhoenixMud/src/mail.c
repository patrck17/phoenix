...
{ 
   ...
   for (tmp = mail_index; (tmp && (str_cmp(tmp->recipient,searchee)!=0));
	tmp = tmp->next)
      ;
...
void index_mail(char *id_to_index, long pos) 
{ 
   ...
   if (!(new_index = find_char_in_index(id_to_index))) 
     ...
     /* name not already in index.. add it */ 
      ...
      new_index->recipient = str_dup(id_to_index); 
...
{ 
   ...
   if (!(mail_file = fopen(MAIL_FILE, "r"))) 
      { 
      log("   Mail file non-existant... creating new file.(%s)",MAIL_FILE); 
      touch(MAIL_FILE);
      return 1; 
      } 
...
{ 
   ...
   if (from ==NULL || to ==NULL || !*message_pointer) 
      { 
      log("SYSERR: Mail system -- non-fatal error #5."); 
      return; 
      } 
   mudlogf(CMP,LVL_IMPL,FALSE,"%s just sent mail to %s",from,to);

   if((fp=fopen(MAIL_LOG,"a+"))==NULL)
      {
      mudlogf(CMP,LVL_IMPL,TRUE,"Could not open %s of append!",
	      MAIL_LOG);
      }
   else
      {
...
/* name is 1st in list */ 
   ...
   *(tmstr + strlen(tmstr) - 1) = '\0'; 
   ...
   buf = get_buffer(200);
   sprintf(buf, " * * * * Phoenix Mud Mail System * * * *\r\n" 
	   "Date: %s\r\n" 
	   "  To: %s\r\n" 
...
/* you'll get no argument from me! */ 
   ...
   if(!str_cmp(buf,"masque"))
      {
      act("$n tells you, 'Masque really HATES mud mail, try sending real "
	  "honest-to-god email to masque@phoenixmud.net.  If that is "
	  "too hard, he doesn't care.'",
	  FALSE,mailman,0,ch,TO_VICT);
      act("$n tells you, 'Oh! He decided you still should be charged "
	  "for the stamp.'",
	  FALSE,mailman,0,ch,TO_VICT);
   ...
   else if(!str_cmp(buf,"ruthven")&&(GET_LEVEL(ch)<LVL_GRGODII))
      {
      act("$n tells you, 'Ruthven really HATES mud mail, try sending real "
	  "honest-to-god email to ruthven@phoenixmud.net.  If that is "
	  "too hard, he doesn't care.'",
	  FALSE,mailman,0,ch,TO_VICT);
      act("$n tells you, 'Oh! He decided you still should be charged "
	  "for the stamp.(twice)'",
	  FALSE,mailman,0,ch,TO_VICT);
      GET_GOLD(ch)-= (STAMP_PRICE*2);
      release_buffer(buf);
      return;
      }
   else if(!str_cmp(buf,"cymynedd"))
      {
      act("$n tells you, 'Cymynedd really HATES mud mail, try sending real "
	  "honest-to-god email to cymynedd@phoenixmud.net.  If that is "
	  "too hard, she doesn't care.'",
	  FALSE,mailman,0,ch,TO_VICT);
      act("$n tells you, 'Oh! She decided you still should be charged "
	  "for the stamp.(twice)'",
	  FALSE,mailman,0,ch,TO_VICT);
      GET_GOLD(ch)-= (STAMP_PRICE*2);
      release_buffer(buf);
   ...
   if(GET_LEVEL(ch)<LVL_IMMORT) 
      ...
      sprintf(buf, "$n tells you, 'I'll take 0 coins for the stamp.'\r\n" 
	      "$n tells you, 'Write your message, (/s saves /h for help)'");
  ...
  /* Start writing! */
   CREATE(writing, char *, 1);
   string_write(ch->desc, writing, MAX_MAIL_SIZE, recipient, NULL);
   release_buffer(buf);
} 
